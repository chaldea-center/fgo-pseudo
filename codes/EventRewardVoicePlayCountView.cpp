void __fastcall EventRewardVoicePlayCountView___ctor(EventRewardVoicePlayCountView_o *this, const MethodInfo *method)
{
  UnityEngine_MonoBehaviour___ctor((UnityEngine_MonoBehaviour_o *)this, 0LL);
}


void __fastcall EventRewardVoicePlayCountView__Close(EventRewardVoicePlayCountView_o *this, const MethodInfo *method)
{
  UILabel_o *countLabel; // x0

  if ( (byte_42B3450 & 1) == 0 )
  {
    sub_B52984(&StringLiteral_1/*""*/);
    byte_42B3450 = 1;
  }
  countLabel = this->fields.countLabel;
  if ( !countLabel
    || (UILabel__set_text(countLabel, (System_String_o *)StringLiteral_1/*""*/, 0LL),
        (countLabel = (UILabel_o *)UnityEngine_Component__get_gameObject((UnityEngine_Component_o *)this, 0LL)) == 0LL) )
  {
    sub_B52A5C(countLabel, method);
  }
  UnityEngine_GameObject__SetActive((UnityEngine_GameObject_o *)countLabel, 0, 0LL);
}


void __fastcall EventRewardVoicePlayCountView__ResetCount(
        EventRewardVoicePlayCountView_o *this,
        const MethodInfo *method)
{
  UILabel_o *countLabel; // x0
  const MethodInfo *v4; // x0
  const MethodInfo *v5; // x1

  if ( (byte_42B3451 & 1) == 0 )
  {
    sub_B52984(&StringLiteral_1/*""*/);
    byte_42B3451 = 1;
  }
  countLabel = this->fields.countLabel;
  if ( !countLabel )
    sub_B52A5C(0LL, method);
  UILabel__set_text(countLabel, (System_String_o *)StringLiteral_1/*""*/, 0LL);
  EventRewardSaveData__DeletePlayCount(v4);
  EventRewardVoicePlayCountView__SetUp(this, v5);
}


void __fastcall EventRewardVoicePlayCountView__SetUp(EventRewardVoicePlayCountView_o *this, const MethodInfo *method)
{
  __int64 gameObject; // x0
  __int64 v4; // x1
  __int64 v5; // x22
  unsigned int v6; // w23
  __int64 v7; // x20
  __int64 v8; // x22
  unsigned int v9; // w28
  __int64 v10; // x20
  int32_t v11; // w24
  int32_t v12; // w25
  int32_t v13; // w23
  __int64 v14; // x26
  __int64 v15; // x27
  int32_t v16; // w0
  const MethodInfo *v17; // x4
  struct UILabel_o *countLabel; // x8
  int32_t EventVoicePlayCount_k__BackingField; // w27
  struct UILabel_o *v20; // x23
  System_Object_array *v21; // x24
  System_String_array **v22; // x2
  System_String_array **v23; // x3
  System_Boolean_array **v24; // x4
  System_Int32_array **v25; // x5
  System_Int32_array *v26; // x6
  System_Int32_array *v27; // x7
  System_Int32_array **v28; // x25
  System_String_array **v29; // x2
  System_String_array **v30; // x3
  System_Boolean_array **v31; // x4
  System_Int32_array **v32; // x5
  System_Int32_array *v33; // x6
  System_Int32_array *v34; // x7
  System_Int32_array **v35; // x25
  System_String_array **v36; // x2
  System_String_array **v37; // x3
  System_Boolean_array **v38; // x4
  System_Int32_array **v39; // x5
  System_Int32_array *v40; // x6
  System_Int32_array *v41; // x7
  System_Int32_array **v42; // x25
  __int64 v43; // x9
  System_String_array **v44; // x2
  System_String_array **v45; // x3
  System_Boolean_array **v46; // x4
  System_Int32_array **v47; // x5
  System_Int32_array *v48; // x6
  System_Int32_array *v49; // x7
  System_Int32_array **v50; // x25
  System_String_array **v51; // x2
  System_String_array **v52; // x3
  System_Boolean_array **v53; // x4
  System_Int32_array **v54; // x5
  System_Int32_array *v55; // x6
  System_Int32_array *v56; // x7
  System_Int32_array **v57; // x25
  System_String_o *mText; // x24
  System_Object_array *v59; // x25
  System_String_array **v60; // x2
  System_String_array **v61; // x3
  System_Boolean_array **v62; // x4
  System_Int32_array **v63; // x5
  System_Int32_array *v64; // x6
  System_Int32_array *v65; // x7
  System_Int32_array **v66; // x26
  System_String_array **v67; // x2
  System_String_array **v68; // x3
  System_Boolean_array **v69; // x4
  System_Int32_array **v70; // x5
  System_Int32_array *v71; // x6
  System_Int32_array *v72; // x7
  System_Int32_array **v73; // x26
  System_String_array **v74; // x2
  System_String_array **v75; // x3
  System_Boolean_array **v76; // x4
  System_Int32_array **v77; // x5
  System_Int32_array *v78; // x6
  System_Int32_array *v79; // x7
  System_Int32_array **v80; // x26
  __int64 v81; // x9
  System_String_array **v82; // x2
  System_String_array **v83; // x3
  System_Boolean_array **v84; // x4
  System_Int32_array **v85; // x5
  System_Int32_array *v86; // x6
  System_Int32_array *v87; // x7
  System_Int32_array **v88; // x26
  System_String_array **v89; // x2
  System_String_array **v90; // x3
  System_Boolean_array **v91; // x4
  System_Int32_array **v92; // x5
  System_Int32_array *v93; // x6
  System_Int32_array *v94; // x7
  System_Int32_array **v95; // x26
  System_String_o *v96; // x0
  __int64 v97; // x0
  __int64 v98; // x0
  __int64 v99; // [xsp+8h] [xbp-98h]
  __int64 v100; // [xsp+10h] [xbp-90h]
  unsigned int v101; // [xsp+1Ch] [xbp-84h]
  __int64 v102; // [xsp+20h] [xbp-80h]
  int32_t v103; // [xsp+2Ch] [xbp-74h] BYREF
  __int64 v104; // [xsp+30h] [xbp-70h] BYREF
  __int64 v105; // [xsp+38h] [xbp-68h]
  int v106; // [xsp+44h] [xbp-5Ch] BYREF
  int v107; // [xsp+48h] [xbp-58h] BYREF
  int v108; // [xsp+4Ch] [xbp-54h] BYREF
  CodeStage_AntiCheat_ObscuredTypes_ObscuredInt_o v109; // 0:x0.16

  if ( (byte_42B344F & 1) == 0 )
  {
    sub_B52984(&Method_DataManager_GetMasterData_EventMaster___);
    sub_B52984(&Method_DataManager_GetMasterData_EventVoicePlayMaster___);
    sub_B52984(&int_TypeInfo);
    sub_B52984(&object___TypeInfo);
    sub_B52984(&CodeStage_AntiCheat_ObscuredTypes_ObscuredInt_TypeInfo);
    sub_B52984(&Method_SingletonMonoBehaviour_DataManager__get_Instance__);
    sub_B52984(&StringLiteral_26/*"\n"*/);
    sub_B52984(&StringLiteral_18607/*"eventId : {0}, slot : {1}, idx : {2}, guideImageId : {3}, count : {4}"*/);
    byte_42B344F = 1;
  }
  gameObject = (__int64)UnityEngine_Component__get_gameObject((UnityEngine_Component_o *)this, 0LL);
  if ( !gameObject )
    goto LABEL_70;
  UnityEngine_GameObject__SetActive((UnityEngine_GameObject_o *)gameObject, 1, 0LL);
  gameObject = (__int64)SingletonMonoBehaviour_WebViewManager___get_Instance((const MethodInfo_2B75DB0 *)Method_SingletonMonoBehaviour_DataManager__get_Instance__);
  if ( !gameObject )
    goto LABEL_70;
  gameObject = (__int64)DataManager__GetMasterData_WarQuestSelectionMaster_(
                          (DataManager_o *)gameObject,
                          (const MethodInfo_1A4F184 *)Method_DataManager_GetMasterData_EventMaster___);
  if ( !gameObject )
    goto LABEL_70;
  gameObject = (__int64)EventMaster__GetEnableExistEventRewardEntityList((EventMaster_o *)gameObject, 0LL);
  if ( gameObject )
  {
    v5 = gameObject;
    v100 = *(_QWORD *)(gameObject + 24);
    if ( v100 )
    {
      if ( (int)v100 >= 1 )
      {
        if ( !(_DWORD)v100 )
          goto LABEL_68;
        v6 = 0;
        v99 = gameObject;
        while ( 1 )
        {
          v7 = *(_QWORD *)(v5 + 8LL * (int)v6 + 32);
          gameObject = (__int64)SingletonMonoBehaviour_WebViewManager___get_Instance((const MethodInfo_2B75DB0 *)Method_SingletonMonoBehaviour_DataManager__get_Instance__);
          if ( !gameObject )
            goto LABEL_70;
          gameObject = (__int64)DataManager__GetMasterData_WarQuestSelectionMaster_(
                                  (DataManager_o *)gameObject,
                                  (const MethodInfo_1A4F184 *)Method_DataManager_GetMasterData_EventVoicePlayMaster___);
          if ( !v7 )
            goto LABEL_70;
          v101 = v6;
          if ( !gameObject )
            goto LABEL_70;
          gameObject = (__int64)EventVoicePlayMaster__GetTargetEventIdEntityList(
                                  (EventVoicePlayMaster_o *)gameObject,
                                  *(_DWORD *)(v7 + 16),
                                  0LL);
          if ( gameObject )
          {
            v8 = gameObject;
            v102 = *(_QWORD *)(gameObject + 24);
            if ( v102 )
            {
              if ( (int)v102 >= 1 )
                break;
            }
          }
LABEL_66:
          v5 = v99;
          v6 = v101 + 1;
          if ( (int)(v101 + 1) >= (int)v100 )
            return;
          if ( v6 >= *(_DWORD *)(v99 + 24) )
            goto LABEL_68;
        }
        if ( !(_DWORD)v102 )
        {
LABEL_68:
          v97 = sub_B52A88(gameObject);
          sub_B52A28(v97, 0LL);
        }
        v9 = 0;
        while ( 1 )
        {
          v10 = *(_QWORD *)(v8 + 8LL * (int)v9 + 32);
          if ( !v10 )
            break;
          v12 = *(_DWORD *)(v10 + 16);
          v11 = *(_DWORD *)(v10 + 20);
          v13 = *(_DWORD *)(v10 + 24);
          v14 = *(_QWORD *)(v10 + 28);
          v15 = *(_QWORD *)(v10 + 36);
          if ( (BYTE3(CodeStage_AntiCheat_ObscuredTypes_ObscuredInt_TypeInfo->vtable._0_Equals.methodPtr) & 4) != 0
            && !CodeStage_AntiCheat_ObscuredTypes_ObscuredInt_TypeInfo->_2.cctor_finished )
          {
            j_il2cpp_runtime_class_init_0(CodeStage_AntiCheat_ObscuredTypes_ObscuredInt_TypeInfo);
          }
          *(_QWORD *)&v109.fields.currentCryptoKey = v14;
          *(_QWORD *)&v109.fields.fakeValue = v15;
          v16 = CodeStage_AntiCheat_ObscuredTypes_ObscuredInt__op_Implicit_44786420(v109, 0LL);
          EventRewardSaveData__LoadEventVoicePlayCount(v12, v11, v13, v16, v17);
          if ( !byte_42B093E )
          {
            gameObject = sub_B52984(&EventRewardSaveData_TypeInfo);
            byte_42B093E = 1;
          }
          countLabel = this->fields.countLabel;
          if ( !countLabel )
            break;
          EventVoicePlayCount_k__BackingField = EventRewardSaveData_TypeInfo->static_fields->_EventVoicePlayCount_k__BackingField;
          gameObject = System_String__IsNullOrEmpty(countLabel->fields.mText, 0LL);
          v20 = this->fields.countLabel;
          if ( (gameObject & 1) != 0 )
          {
            v21 = (System_Object_array *)sub_B5299C(object___TypeInfo, 5LL);
            v108 = *(_DWORD *)(v10 + 16);
            gameObject = j_il2cpp_value_box_0(int_TypeInfo, &v108);
            if ( !v21 )
              break;
            v28 = (System_Int32_array **)gameObject;
            if ( gameObject )
            {
              gameObject = sub_B52A44(gameObject, v21->obj.klass->_1.element_class);
              if ( !gameObject )
                goto LABEL_71;
            }
            if ( !v21->max_length )
              goto LABEL_68;
            v21->m_Items[0] = (Il2CppObject *)v28;
            sub_B52920((BattleServantConfConponent_o *)v21->m_Items, v28, v22, v23, v24, v25, v26, v27);
            v107 = *(_DWORD *)(v10 + 20);
            gameObject = j_il2cpp_value_box_0(int_TypeInfo, &v107);
            v35 = (System_Int32_array **)gameObject;
            if ( gameObject )
            {
              gameObject = sub_B52A44(gameObject, v21->obj.klass->_1.element_class);
              if ( !gameObject )
                goto LABEL_71;
            }
            if ( v21->max_length <= 1 )
              goto LABEL_68;
            v21->m_Items[1] = (Il2CppObject *)v35;
            sub_B52920((BattleServantConfConponent_o *)&v21->m_Items[1], v35, v29, v30, v31, v32, v33, v34);
            v106 = *(_DWORD *)(v10 + 24);
            gameObject = j_il2cpp_value_box_0(int_TypeInfo, &v106);
            v42 = (System_Int32_array **)gameObject;
            if ( gameObject )
            {
              gameObject = sub_B52A44(gameObject, v21->obj.klass->_1.element_class);
              if ( !gameObject )
                goto LABEL_71;
            }
            if ( v21->max_length <= 2 )
              goto LABEL_68;
            v21->m_Items[2] = (Il2CppObject *)v42;
            sub_B52920((BattleServantConfConponent_o *)&v21->m_Items[2], v42, v36, v37, v38, v39, v40, v41);
            v43 = *(_QWORD *)(v10 + 36);
            v104 = *(_QWORD *)(v10 + 28);
            v105 = v43;
            gameObject = j_il2cpp_value_box_0(CodeStage_AntiCheat_ObscuredTypes_ObscuredInt_TypeInfo, &v104);
            v50 = (System_Int32_array **)gameObject;
            if ( gameObject )
            {
              gameObject = sub_B52A44(gameObject, v21->obj.klass->_1.element_class);
              if ( !gameObject )
                goto LABEL_71;
            }
            if ( v21->max_length <= 3 )
              goto LABEL_68;
            v21->m_Items[3] = (Il2CppObject *)v50;
            sub_B52920((BattleServantConfConponent_o *)&v21->m_Items[3], v50, v44, v45, v46, v47, v48, v49);
            v103 = EventVoicePlayCount_k__BackingField;
            gameObject = j_il2cpp_value_box_0(int_TypeInfo, &v103);
            v57 = (System_Int32_array **)gameObject;
            if ( gameObject )
            {
              gameObject = sub_B52A44(gameObject, v21->obj.klass->_1.element_class);
              if ( !gameObject )
                goto LABEL_71;
            }
            if ( v21->max_length <= 4 )
              goto LABEL_68;
            v21->m_Items[4] = (Il2CppObject *)v57;
            sub_B52920((BattleServantConfConponent_o *)&v21->m_Items[4], v57, v51, v52, v53, v54, v55, v56);
            gameObject = (__int64)System_String__Format_44647040((System_String_o *)StringLiteral_18607/*"eventId : {0}, slot : {1}, idx : {2}, guideImageId : {3}, count : {4}"*/, v21, 0LL);
            if ( !v20 )
              break;
          }
          else
          {
            if ( !v20 )
              break;
            mText = v20->fields.mText;
            v59 = (System_Object_array *)sub_B5299C(object___TypeInfo, 5LL);
            v108 = *(_DWORD *)(v10 + 16);
            gameObject = j_il2cpp_value_box_0(int_TypeInfo, &v108);
            if ( !v59 )
              break;
            v66 = (System_Int32_array **)gameObject;
            if ( gameObject )
            {
              gameObject = sub_B52A44(gameObject, v59->obj.klass->_1.element_class);
              if ( !gameObject )
                goto LABEL_71;
            }
            if ( !v59->max_length )
              goto LABEL_68;
            v59->m_Items[0] = (Il2CppObject *)v66;
            sub_B52920((BattleServantConfConponent_o *)v59->m_Items, v66, v60, v61, v62, v63, v64, v65);
            v107 = *(_DWORD *)(v10 + 20);
            gameObject = j_il2cpp_value_box_0(int_TypeInfo, &v107);
            v73 = (System_Int32_array **)gameObject;
            if ( gameObject )
            {
              gameObject = sub_B52A44(gameObject, v59->obj.klass->_1.element_class);
              if ( !gameObject )
                goto LABEL_71;
            }
            if ( v59->max_length <= 1 )
              goto LABEL_68;
            v59->m_Items[1] = (Il2CppObject *)v73;
            sub_B52920((BattleServantConfConponent_o *)&v59->m_Items[1], v73, v67, v68, v69, v70, v71, v72);
            v106 = *(_DWORD *)(v10 + 24);
            gameObject = j_il2cpp_value_box_0(int_TypeInfo, &v106);
            v80 = (System_Int32_array **)gameObject;
            if ( gameObject )
            {
              gameObject = sub_B52A44(gameObject, v59->obj.klass->_1.element_class);
              if ( !gameObject )
                goto LABEL_71;
            }
            if ( v59->max_length <= 2 )
              goto LABEL_68;
            v59->m_Items[2] = (Il2CppObject *)v80;
            sub_B52920((BattleServantConfConponent_o *)&v59->m_Items[2], v80, v74, v75, v76, v77, v78, v79);
            v81 = *(_QWORD *)(v10 + 36);
            v104 = *(_QWORD *)(v10 + 28);
            v105 = v81;
            gameObject = j_il2cpp_value_box_0(CodeStage_AntiCheat_ObscuredTypes_ObscuredInt_TypeInfo, &v104);
            v88 = (System_Int32_array **)gameObject;
            if ( gameObject )
            {
              gameObject = sub_B52A44(gameObject, v59->obj.klass->_1.element_class);
              if ( !gameObject )
                goto LABEL_71;
            }
            if ( v59->max_length <= 3 )
              goto LABEL_68;
            v59->m_Items[3] = (Il2CppObject *)v88;
            sub_B52920((BattleServantConfConponent_o *)&v59->m_Items[3], v88, v82, v83, v84, v85, v86, v87);
            v103 = EventVoicePlayCount_k__BackingField;
            gameObject = j_il2cpp_value_box_0(int_TypeInfo, &v103);
            v95 = (System_Int32_array **)gameObject;
            if ( gameObject )
            {
              gameObject = sub_B52A44(gameObject, v59->obj.klass->_1.element_class);
              if ( !gameObject )
              {
LABEL_71:
                v98 = sub_B52A7C(gameObject);
                sub_B52A28(v98, 0LL);
              }
            }
            if ( v59->max_length <= 4 )
              goto LABEL_68;
            v59->m_Items[4] = (Il2CppObject *)v95;
            sub_B52920((BattleServantConfConponent_o *)&v59->m_Items[4], v95, v89, v90, v91, v92, v93, v94);
            v96 = System_String__Format_44647040((System_String_o *)StringLiteral_18607/*"eventId : {0}, slot : {1}, idx : {2}, guideImageId : {3}, count : {4}"*/, v59, 0LL);
            gameObject = (__int64)System_String__Concat_44570600(mText, (System_String_o *)StringLiteral_26/*"\n"*/, v96, 0LL);
          }
          UILabel__set_text(v20, (System_String_o *)gameObject, 0LL);
          if ( (int)++v9 >= (int)v102 )
            goto LABEL_66;
          if ( v9 >= *(_DWORD *)(v8 + 24) )
            goto LABEL_68;
        }
LABEL_70:
        sub_B52A5C(gameObject, v4);
      }
    }
  }
}