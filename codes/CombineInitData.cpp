void __fastcall CombineInitData___ctor(CombineInitData_o *this, const MethodInfo *method)
{
  __int64 v2; // x2
  __int64 v3; // x3
  __int64 v5; // x1
  __int64 v6; // x2
  System_Collections_Generic_List_int__o *v7; // x20
  int64_t v8; // x2
  int32_t v9; // w3
  System_String_o *v10; // x4
  BattleSetupInfo_o *v11; // x5
  FollowerInfo_o *v12; // x6
  PartyListViewItem_o *v13; // x7

  if ( (byte_4B1998D & 1) == 0 )
  {
    sub_1BCA7E0(&Method_System_Collections_Generic_List_int___ctor__, method, v2);
    sub_1BCA7E0(&System_Collections_Generic_List_int__TypeInfo, v5, v6);
    byte_4B1998D = 1;
  }
  v7 = (System_Collections_Generic_List_int__o *)sub_1BCAA2C(
                                                   System_Collections_Generic_List_int__TypeInfo,
                                                   method,
                                                   v2,
                                                   v3);
  System_Collections_Generic_List_int____ctor(
    v7,
    (const MethodInfo_35843E4 *)Method_System_Collections_Generic_List_int___ctor__);
  this->fields.eventIdList = v7;
  sub_1BCA784((PartyOrganizationUtility_o *)&this->fields.eventIdList, (int64_t)v7, v8, v9, v10, v11, v12, v13);
  UnityEngine_MonoBehaviour___ctor((UnityEngine_MonoBehaviour_o *)this, 0LL);
}


// local variable allocation has failed, the output may be wrong!
EventCampaignEntity_o *__fastcall CombineInitData__getCombineEventData(
        CombineInitData_o *this,
        int32_t eventId,
        const MethodInfo *method)
{
  __int64 v5; // x1
  __int64 v6; // x2
  __int64 v7; // x1
  __int64 v8; // x2
  DataManager_o *Instance; // x0
  __int64 v10; // x1
  int m_CancellationTokenSource; // w8
  __int64 v12; // x9
  EventCampaignEntity_o *v13; // x21
  __int64 v15; // x8
  _QWORD *v16; // x9
  __int64 m_CancellationTokenSource_low; // x10

  if ( (byte_4B1998C & 1) == 0 )
  {
    sub_1BCA7E0(&Method_DataManager_getEntitys_EventCampaignMaster__EventCampaignEntity___, *(_QWORD *)&eventId, method);
    sub_1BCA7E0(&Method_System_Collections_Generic_List_int__Add__, v5, v6);
    sub_1BCA7E0(&Method_SingletonMonoBehaviour_DataManager__get_Instance__, v7, v8);
    byte_4B1998C = 1;
  }
  Instance = (DataManager_o *)SingletonMonoBehaviour_object___get_Instance((const MethodInfo_37DEB14 *)Method_SingletonMonoBehaviour_DataManager__get_Instance__);
  if ( !Instance )
    goto LABEL_18;
  Instance = (DataManager_o *)DataManager__getEntitys_object__object_(
                                Instance,
                                (const MethodInfo_2F12EF0 *)Method_DataManager_getEntitys_EventCampaignMaster__EventCampaignEntity___);
  if ( !Instance )
    return 0LL;
  m_CancellationTokenSource = (int)Instance->fields.m_CancellationTokenSource;
  if ( m_CancellationTokenSource < 1 )
    return 0LL;
  v12 = 0LL;
  while ( 1 )
  {
    if ( (unsigned int)v12 >= m_CancellationTokenSource )
      sub_1BCAA44(Instance, v10);
    v13 = (EventCampaignEntity_o *)*((_QWORD *)&Instance->fields._DispLog + v12);
    if ( !v13 )
      goto LABEL_18;
    if ( v13->fields.eventId == eventId )
      break;
    if ( (int)++v12 >= m_CancellationTokenSource )
      return 0LL;
  }
  Instance = (DataManager_o *)this->fields.eventIdList;
  if ( !Instance
    || (v15 = *(_QWORD *)&Instance->fields.m_CachedPtr,
        v16 = Method_System_Collections_Generic_List_int__Add__,
        ++HIDWORD(Instance->fields.m_CancellationTokenSource),
        !v15) )
  {
LABEL_18:
    sub_1BCAA3C(Instance, v10);
  }
  m_CancellationTokenSource_low = SLODWORD(Instance->fields.m_CancellationTokenSource);
  if ( (unsigned int)m_CancellationTokenSource_low >= *(_DWORD *)(v15 + 24) )
  {
    System_Collections_Generic_List_int___AddWithResize(
      (System_Collections_Generic_List_int__o *)Instance,
      eventId,
      *(const MethodInfo_3584C38 **)(*(_QWORD *)(v16[4] + 192LL) + 112LL));
  }
  else
  {
    LODWORD(Instance->fields.m_CancellationTokenSource) = m_CancellationTokenSource_low + 1;
    *(_DWORD *)(v15 + 4 * m_CancellationTokenSource_low + 32) = eventId;
  }
  return v13;
}


EventEntity_array *__fastcall CombineInitData__getCombineEventList(CombineInitData_o *this, const MethodInfo *method)
{
  return this->fields.combineEventList;
}


void __fastcall CombineInitData__getEventData(CombineInitData_o *this, const MethodInfo *method)
{
  __int64 v2; // x2
  __int64 v4; // x1
  __int64 v5; // x2
  __int64 v6; // x1
  __int64 v7; // x2
  __int64 v8; // x1
  __int64 v9; // x2
  __int64 v10; // x1
  __int64 v11; // x2
  __int64 v12; // x1
  __int64 v13; // x2
  EventEntity_array *Instance; // x0
  __int64 v15; // x1
  EventMaster_o *v16; // x21
  EventEntity_array *EnableEntityList; // x20
  __int64 v18; // x2
  __int64 v19; // x3
  EventEntity_array *v20; // x21
  System_Collections_Generic_List_object__o *v21; // x22
  struct EventEntity_array *v22; // x0
  int64_t v23; // x2
  int32_t v24; // w3
  System_String_o *v25; // x4
  BattleSetupInfo_o *v26; // x5
  FollowerInfo_o *v27; // x6
  PartyListViewItem_o *v28; // x7

  if ( (byte_4B1998B & 1) == 0 )
  {
    sub_1BCA7E0(&Method_DataManager_GetMasterData_EventMaster___, method, v2);
    sub_1BCA7E0(&Method_System_Collections_Generic_List_EventEntity__AddRange__, v4, v5);
    sub_1BCA7E0(&Method_System_Collections_Generic_List_EventEntity__ToArray__, v6, v7);
    sub_1BCA7E0(&Method_System_Collections_Generic_List_EventEntity___ctor___76780568, v8, v9);
    sub_1BCA7E0(&System_Collections_Generic_List_EventEntity__TypeInfo, v10, v11);
    sub_1BCA7E0(&Method_SingletonMonoBehaviour_DataManager__get_Instance__, v12, v13);
    byte_4B1998B = 1;
  }
  Instance = (EventEntity_array *)SingletonMonoBehaviour_object___get_Instance((const MethodInfo_37DEB14 *)Method_SingletonMonoBehaviour_DataManager__get_Instance__);
  if ( !Instance
    || (Instance = (EventEntity_array *)DataManager__GetMasterData_object_(
                                          (DataManager_o *)Instance,
                                          (const MethodInfo_2F12C90 *)Method_DataManager_GetMasterData_EventMaster___)) == 0LL
    || (v16 = (EventMaster_o *)Instance,
        EnableEntityList = EventMaster__GetEnableEntityList((EventMaster_o *)Instance, 5, 1, 0LL),
        Instance = EventMaster__GetEnableEntityList(v16, 13, 1, 0LL),
        !EnableEntityList)
    || (v20 = Instance) == 0LL
    || (v21 = (System_Collections_Generic_List_object__o *)sub_1BCAA2C(
                                                             System_Collections_Generic_List_EventEntity__TypeInfo,
                                                             v15,
                                                             v18,
                                                             v19),
        System_Collections_Generic_List_object____ctor_56235160(
          v21,
          v20->max_length + EnableEntityList->max_length,
          (const MethodInfo_35A1498 *)Method_System_Collections_Generic_List_EventEntity___ctor___76780568),
        !v21) )
  {
    sub_1BCAA3C(Instance, v15);
  }
  System_Collections_Generic_List_object___AddRange(
    v21,
    (System_Collections_Generic_IEnumerable_T__o *)EnableEntityList,
    (const MethodInfo_35A1E68 *)Method_System_Collections_Generic_List_EventEntity__AddRange__);
  System_Collections_Generic_List_object___AddRange(
    v21,
    (System_Collections_Generic_IEnumerable_T__o *)v20,
    (const MethodInfo_35A1E68 *)Method_System_Collections_Generic_List_EventEntity__AddRange__);
  v22 = (struct EventEntity_array *)System_Collections_Generic_List_object___ToArray(
                                      v21,
                                      (const MethodInfo_35A37B4 *)Method_System_Collections_Generic_List_EventEntity__ToArray__);
  this->fields.combineEventList = v22;
  sub_1BCA784((PartyOrganizationUtility_o *)&this->fields.combineEventList, (int64_t)v22, v23, v24, v25, v26, v27, v28);
}


System_Collections_Generic_List_int__o *__fastcall CombineInitData__getEventIdList(
        CombineInitData_o *this,
        const MethodInfo *method)
{
  return this->fields.eventIdList;
}