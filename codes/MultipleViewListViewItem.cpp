...
// local variable allocation has failed, the output may be wrong!
void __fastcall MultipleViewListViewItem__ModifyItem_28741316(
        MultipleViewListViewItem_o *this,
        int32_t selectIndex,
        ...
        const MethodInfo *method)
{
  ...
  if ( (byte_40FC00B & 1) == 0 )
  {
    sub_B16FFC(&MultipleViewListViewManager_TypeInfo, sort);
...
LABEL_67:
          ...
          case 0xE:
            ...
            if ( !manager )
              goto LABEL_69;
            v42 = *(&MultipleViewListViewManager_TypeInfo->_2.bitflags2 + 1);
            if ( *(&manager->klass->_2.bitflags2 + 1) < (unsigned int)v42
              || (MultipleViewListViewManager_c *)manager->klass->_2.typeHierarchy[v42 - 1] != MultipleViewListViewManager_TypeInfo )
            {
              goto LABEL_69;
            }
            amountSortValue = MultipleViewListViewManager__GetAmountSortValue(
                                (MultipleViewListViewManager_o *)manager,
                                this->fields.svtId,
                                v11);
