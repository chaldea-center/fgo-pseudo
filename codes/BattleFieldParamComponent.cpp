...
{
  *(_OWORD *)&this->fields.RAID_POSITION.fields.x = xmmword_BDE710;
...
void __fastcall BattleFieldParamComponent__InitUpdateView(BattleFieldParamComponent_o *this, const MethodInfo *method)
{
  ...
  if ( this->fields.data )
  {
    ...
    if ( root )
    {
      ...
      if ( !byte_4AFBDB1 )
      {
        root = sub_1BC3008(&UnityEngine_Vector3_TypeInfo, method);
        byte_4AFBDB1 = 1;
      }
      if ( v5 )
      {
            ...
            if ( battle_ent )
            {
              ...
              if ( battleInfo )
              {
                ...
                if ( raidInfo && *(_QWORD *)&raidInfo->max_length )
                {
                  ...
                  if ( (root & 1) == 0 )
                  {
                    root = (__int64)SingletonMonoBehaviour_object___get_Instance((const MethodInfo_38DFD48 *)Method_SingletonMonoBehaviour_DataManager__get_Instance__);
                    if ( !root )
                      goto LABEL_34;
        ...
        const MethodInfo *method)
{
  ...
  sub_1BC2FAC((CGThumbnailListItem_o *)&this->fields.perf, (int32_t)Perf_k__BackingField, v8, v9);
  ...
  if ( ((unsigned __int8)ShowServantParam & 1) == 0 )
  {
    if ( !buffData
      || (v8 = this->fields.showBuffComponent, ShowServantParam = BattleBuffData__getShowServantParam(buffData, 0LL),
                                               !v8) )
    {
      sub_1BC3264(ShowServantParam, v7);
    }
