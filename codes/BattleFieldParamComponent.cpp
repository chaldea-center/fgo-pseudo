...
{
  *(_OWORD *)&this->fields.RAID_POSITION.fields.x = xmmword_BFA440;
...
void __fastcall BattleFieldParamComponent__InitUpdateView(BattleFieldParamComponent_o *this, const MethodInfo *method)
{
  ...
  if ( this->fields.data )
  {
    ...
    if ( root )
    {
      ...
      if ( !byte_4BC2141 )
      {
        root = sub_1C1ABD4(&UnityEngine_Vector3_TypeInfo, method);
        byte_4BC2141 = 1;
      }
      if ( v5 )
      {
            ...
            if ( battle_ent )
            {
              ...
              if ( battleInfo )
              {
                ...
                if ( raidInfo && *(_QWORD *)&raidInfo->max_length )
                {
                  ...
                  if ( (root & 1) == 0 )
                  {
                    root = (__int64)SingletonMonoBehaviour_object___get_Instance((const MethodInfo_387A8A8 *)Method_SingletonMonoBehaviour_DataManager__get_Instance__);
                    if ( !root )
                      goto LABEL_34;
  ...
  if ( ((unsigned __int8)ShowServantParam & 1) == 0 )
  {
    if ( !buffData
      || (v8 = this->fields.showBuffComponent, ShowServantParam = BattleBuffData__getShowServantParam(buffData, 0LL),
                                               !v8) )
    {
      sub_1C1AE30(ShowServantParam, v7);
    }
