void __fastcall ExUISpriteRenderer___cctor(const MethodInfo *method)
{
  __int64 v1; // x1
  __int64 v2; // x2

  if ( (byte_4B1567E & 1) == 0 )
  {
    sub_1BCA7E0(&ExUISpriteRenderer_TypeInfo, v1, v2);
    byte_4B1567E = 1;
  }
  *ExUISpriteRenderer_TypeInfo->static_fields = (struct ExUISpriteRenderer_StaticFields)xmmword_BD3000;
}


void __fastcall ExUISpriteRenderer___ctor(ExUISpriteRenderer_o *this, const MethodInfo *method)
{
  *(_QWORD *)&this->fields.oldWidth = 0x100000001LL;
  UITweenRenderer___ctor((UITweenRenderer_o *)this, 0LL);
}


void __fastcall ExUISpriteRenderer__ClearImage(ExUISpriteRenderer_o *this, const MethodInfo *method)
{
  __int64 v2; // x2
  __int64 v4; // x1
  __int64 v5; // x2
  UnityEngine_SpriteRenderer_o *spriteRenderer; // x0
  AssetData_o *assetData; // x20
  int64_t v8; // x2
  int32_t v9; // w3
  System_String_o *v10; // x4
  BattleSetupInfo_o *v11; // x5
  FollowerInfo_o *v12; // x6
  PartyListViewItem_o *v13; // x7
  System_String_o *loadAssetName; // x20
  int64_t v15; // x2
  int32_t v16; // w3
  System_String_o *v17; // x4
  BattleSetupInfo_o *v18; // x5
  FollowerInfo_o *v19; // x6
  PartyListViewItem_o *v20; // x7
  struct System_Action_o *callbackFunc; // x20
  int64_t v22; // x2
  int32_t v23; // w3
  System_String_o *v24; // x4
  BattleSetupInfo_o *v25; // x5
  FollowerInfo_o *v26; // x6
  PartyListViewItem_o *v27; // x7

  if ( (byte_4B15675 & 1) == 0 )
  {
    sub_1BCA7E0(&AssetManager_TypeInfo, method, v2);
    sub_1BCA7E0(&StringLiteral_16522/*"_SubTex"*/, v4, v5);
    byte_4B15675 = 1;
  }
  spriteRenderer = this->fields.spriteRenderer;
  if ( !spriteRenderer )
    goto LABEL_18;
  UnityEngine_SpriteRenderer__set_sprite(spriteRenderer, 0LL, 0LL);
  if ( !this->fields.isTextureAlphaImage )
    goto LABEL_8;
  spriteRenderer = this->fields.spriteRenderer;
  this->fields.isTextureAlphaImage = 0;
  if ( !spriteRenderer
    || (spriteRenderer = (UnityEngine_SpriteRenderer_o *)UnityEngine_Renderer__get_material(
                                                           (UnityEngine_Renderer_o *)spriteRenderer,
                                                           0LL)) == 0LL )
  {
LABEL_18:
    sub_1BCAA3C(spriteRenderer, method);
  }
  UnityEngine_Material__SetTexture(
    (UnityEngine_Material_o *)spriteRenderer,
    (System_String_o *)StringLiteral_16522/*"_SubTex"*/,
    0LL,
    0LL);
LABEL_8:
  assetData = this->fields.assetData;
  if ( assetData )
  {
    if ( !AssetManager_TypeInfo->_2.cctor_finished )
      j_il2cpp_runtime_class_init_0(AssetManager_TypeInfo, method);
    AssetManager__releaseAsset_38505704(assetData, 0LL);
    this->fields.assetData = 0LL;
    sub_1BCA784((PartyOrganizationUtility_o *)&this->fields.assetData, 0LL, v8, v9, v10, v11, v12, v13);
  }
  loadAssetName = this->fields.loadAssetName;
  if ( loadAssetName )
  {
    if ( !AssetManager_TypeInfo->_2.cctor_finished )
      j_il2cpp_runtime_class_init_0(AssetManager_TypeInfo, method);
    AssetManager__releaseAssetStorage(loadAssetName, 0LL);
    this->fields.loadAssetName = 0LL;
    sub_1BCA784((PartyOrganizationUtility_o *)&this->fields.loadAssetName, 0LL, v15, v16, v17, v18, v19, v20);
    callbackFunc = this->fields.callbackFunc;
    this->fields.callbackFunc = 0LL;
    sub_1BCA784((PartyOrganizationUtility_o *)&this->fields.callbackFunc, 0LL, v22, v23, v24, v25, v26, v27);
    if ( callbackFunc )
      ((void (__fastcall *)(struct System_Reflection_MethodInfo_o *, _QWORD))callbackFunc->fields.m_target)(
        callbackFunc->fields.original_method_info,
        *(_QWORD *)&callbackFunc->fields.extra_arg);
  }
}


void __fastcall ExUISpriteRenderer__EndLoad(ExUISpriteRenderer_o *this, AssetData_o *data, const MethodInfo *method)
{
  System_String_o *loadAssetName; // x21
  int64_t v6; // x2
  int32_t v7; // w3
  System_String_o *v8; // x4
  BattleSetupInfo_o *v9; // x5
  FollowerInfo_o *v10; // x6
  PartyListViewItem_o *v11; // x7
  AssetData_o *assetData; // x21
  int64_t v13; // x2
  int32_t v14; // w3
  System_String_o *v15; // x4
  BattleSetupInfo_o *v16; // x5
  FollowerInfo_o *v17; // x6
  PartyListViewItem_o *v18; // x7
  const MethodInfo *v19; // x1
  __int64 v20; // x1
  int64_t v21; // x2
  int32_t v22; // w3
  System_String_o *v23; // x4
  BattleSetupInfo_o *v24; // x5
  FollowerInfo_o *v25; // x6
  PartyListViewItem_o *v26; // x7
  struct System_Action_o *callbackFunc; // x20

  if ( (byte_4B1567A & 1) == 0 )
  {
    sub_1BCA7E0(&AssetManager_TypeInfo, data, method);
    byte_4B1567A = 1;
  }
  if ( data )
  {
    loadAssetName = this->fields.loadAssetName;
    if ( loadAssetName )
    {
      if ( !AssetManager_TypeInfo->_2.cctor_finished )
        j_il2cpp_runtime_class_init_0(AssetManager_TypeInfo, data);
      if ( AssetManager__compAssetStorage_38503380(data, loadAssetName, 0LL) )
      {
        assetData = this->fields.assetData;
        this->fields.loadAssetName = 0LL;
        sub_1BCA784((PartyOrganizationUtility_o *)&this->fields.loadAssetName, 0LL, v6, v7, v8, v9, v10, v11);
        this->fields.assetData = data;
        sub_1BCA784((PartyOrganizationUtility_o *)&this->fields.assetData, (int64_t)data, v13, v14, v15, v16, v17, v18);
        ExUISpriteRenderer__SetAssetTexture(this, v19);
        if ( assetData )
        {
          if ( !AssetManager_TypeInfo->_2.cctor_finished )
            j_il2cpp_runtime_class_init_0(AssetManager_TypeInfo, v20);
          AssetManager__releaseAsset_38505704(assetData, 0LL);
        }
        callbackFunc = this->fields.callbackFunc;
        this->fields.callbackFunc = 0LL;
        sub_1BCA784((PartyOrganizationUtility_o *)&this->fields.callbackFunc, 0LL, v21, v22, v23, v24, v25, v26);
        if ( callbackFunc )
          ((void (__fastcall *)(struct System_Reflection_MethodInfo_o *, _QWORD))callbackFunc->fields.m_target)(
            callbackFunc->fields.original_method_info,
            *(_QWORD *)&callbackFunc->fields.extra_arg);
      }
    }
  }
}


UnityEngine_Color_o __fastcall ExUISpriteRenderer__GetTweenColor(ExUISpriteRenderer_o *this, const MethodInfo *method)
{
  float r; // s0
  float g; // s1
  float b; // s2
  float a; // s3
  UnityEngine_Color_o result; // 0:s0.4,4:s1.4,8:s2.4,12:s3.4

  r = this->fields.mColor.fields.r;
  g = this->fields.mColor.fields.g;
  b = this->fields.mColor.fields.b;
  a = this->fields.mColor.fields.a;
  result.fields.a = a;
  result.fields.b = b;
  result.fields.g = g;
  result.fields.r = r;
  return result;
}


void __fastcall ExUISpriteRenderer__InitBaseScale(ExUISpriteRenderer_o *this, const MethodInfo *method)
{
  UnityEngine_Transform_o *transform; // x0
  __int64 v4; // x1

  if ( !this->fields.isInit )
  {
    this->fields.isInit = 1;
    transform = UnityEngine_Component__get_transform((UnityEngine_Component_o *)this, 0LL);
    if ( !transform )
      sub_1BCAA3C(0LL, v4);
    this->fields.baseScale = UnityEngine_Transform__get_localScale(transform, 0LL);
  }
}


void __fastcall ExUISpriteRenderer__LoadAssetTexture(
        ExUISpriteRenderer_o *this,
        System_String_o *assetName,
        System_String_o *assetLabel,
        bool isUseAlphaImage,
        System_Action_o *callback,
        const MethodInfo *method)
{
  FollowerInfo_o *v6; // x6
  PartyListViewItem_o *v7; // x7
  bool v12; // w25
  __int64 v13; // x1
  __int64 v14; // x2
  __int64 v15; // x1
  __int64 v16; // x2
  __int64 v17; // x1
  __int64 v18; // x2
  __int64 v19; // x1
  int64_t v20; // x2
  char v21; // w3
  System_String_o *v22; // x4
  BattleSetupInfo_o *v23; // x5
  FollowerInfo_o *v24; // x6
  PartyListViewItem_o *v25; // x7
  System_String_o *loadAssetName; // x23
  struct System_String_o **p_loadAssetName; // x22
  __int64 v28; // x1
  System_Delegate_o *callbackFunc; // x0
  struct System_Action_o **p_callbackFunc; // x19
  System_Delegate_o *v31; // x0
  int64_t v32; // x2
  char v33; // w3
  System_String_o *v34; // x4
  BattleSetupInfo_o *v35; // x5
  FollowerInfo_o *v36; // x6
  PartyListViewItem_o *v37; // x7
  System_Delegate_o *v38; // x8
  System_Action_c *v39; // x1
  AssetData_o *assetData; // x23
  const MethodInfo *v41; // x1
  struct System_Action_o *v42; // x8
  System_String_o *v43; // x23
  __int64 v44; // x1
  __int64 v45; // x2
  __int64 v46; // x3
  struct System_Action_o **v47; // x22
  System_Delegate_o *v48; // x0
  int64_t v49; // x2
  char v50; // w3
  System_String_o *v51; // x4
  BattleSetupInfo_o *v52; // x5
  FollowerInfo_o *v53; // x6
  PartyListViewItem_o *v54; // x7
  System_Action_c *v55; // x1
  AssetLoader_LoadEndDataHandler_o *v56; // x21
  __int64 v57; // x1

  v12 = isUseAlphaImage;
  if ( (byte_4B15679 & 1) == 0 )
  {
    sub_1BCA7E0(&System_Action_TypeInfo, assetName, assetLabel);
    sub_1BCA7E0(&AssetManager_TypeInfo, v13, v14);
    sub_1BCA7E0(&Method_ExUISpriteRenderer_EndLoad__, v15, v16);
    sub_1BCA7E0(&AssetLoader_LoadEndDataHandler_TypeInfo, v17, v18);
    byte_4B15679 = 1;
  }
  this->fields.assetLabel = assetLabel;
  sub_1BCA784(
    (PartyOrganizationUtility_o *)&this->fields.assetLabel,
    (int64_t)assetLabel,
    (int64_t)assetLabel,
    isUseAlphaImage,
    (System_String_o *)callback,
    (BattleSetupInfo_o *)method,
    v6,
    v7);
  loadAssetName = this->fields.loadAssetName;
  p_loadAssetName = &this->fields.loadAssetName;
  this->fields.isUseAlphaImage = v12;
  if ( loadAssetName )
  {
    if ( !AssetManager_TypeInfo->_2.cctor_finished )
      j_il2cpp_runtime_class_init_0(AssetManager_TypeInfo, v19);
    if ( AssetManager__compAssetStorage(loadAssetName, assetName, 0LL) )
    {
      callbackFunc = (System_Delegate_o *)this->fields.callbackFunc;
      if ( !callbackFunc )
        return;
      p_callbackFunc = &this->fields.callbackFunc;
      v31 = System_Delegate__Combine(callbackFunc, (System_Delegate_o *)this->fields.callbackFunc, 0LL);
      v38 = v31;
      if ( v31 )
      {
        v39 = System_Action_TypeInfo;
        if ( (System_Action_c *)v31->klass != System_Action_TypeInfo
          || (*p_callbackFunc = (struct System_Action_o *)v31, (System_Action_c *)v31->klass != v39) )
        {
LABEL_25:
          sub_1BCACFC(v38);
          return;
        }
      }
      else
      {
        *p_callbackFunc = 0LL;
      }
      sub_1BCA784((PartyOrganizationUtility_o *)&this->fields.callbackFunc, (int64_t)v31, v32, v33, v34, v35, v36, v37);
      return;
    }
    v43 = *p_loadAssetName;
    if ( !AssetManager_TypeInfo->_2.cctor_finished )
      j_il2cpp_runtime_class_init_0(AssetManager_TypeInfo, v28);
    AssetManager__releaseAssetStorage(v43, 0LL);
  }
  else
  {
    assetData = this->fields.assetData;
    if ( assetData )
    {
      if ( !AssetManager_TypeInfo->_2.cctor_finished )
        j_il2cpp_runtime_class_init_0(AssetManager_TypeInfo, v19);
      if ( AssetManager__compAssetStorage_38503380(assetData, assetName, 0LL) )
      {
        ExUISpriteRenderer__SetAssetTexture(this, v41);
        v42 = this->fields.callbackFunc;
        if ( v42 )
          ((void (__fastcall *)(struct System_Reflection_MethodInfo_o *, _QWORD))v42->fields.m_target)(
            v42->fields.original_method_info,
            *(_QWORD *)&v42->fields.extra_arg);
        return;
      }
    }
  }
  *p_loadAssetName = assetName;
  sub_1BCA784(
    (PartyOrganizationUtility_o *)&this->fields.loadAssetName,
    (int64_t)assetName,
    v20,
    v21,
    v22,
    v23,
    v24,
    v25);
  if ( callback )
  {
    v47 = &this->fields.callbackFunc;
    v48 = System_Delegate__Combine((System_Delegate_o *)this->fields.callbackFunc, (System_Delegate_o *)callback, 0LL);
    v38 = v48;
    if ( v48 )
    {
      v55 = System_Action_TypeInfo;
      if ( (System_Action_c *)v48->klass != System_Action_TypeInfo )
        goto LABEL_25;
      *v47 = (struct System_Action_o *)v48;
      if ( (System_Action_c *)v48->klass != v55 )
        goto LABEL_25;
    }
    else
    {
      *v47 = 0LL;
    }
    sub_1BCA784((PartyOrganizationUtility_o *)&this->fields.callbackFunc, (int64_t)v48, v49, v50, v51, v52, v53, v54);
  }
  v56 = (AssetLoader_LoadEndDataHandler_o *)sub_1BCAA2C(AssetLoader_LoadEndDataHandler_TypeInfo, v44, v45, v46);
  AssetLoader_LoadEndDataHandler___ctor(v56, (Il2CppObject *)this, Method_ExUISpriteRenderer_EndLoad__, 0LL);
  if ( !AssetManager_TypeInfo->_2.cctor_finished )
    j_il2cpp_runtime_class_init_0(AssetManager_TypeInfo, v57);
  AssetManager__loadAssetStorage(assetName, v56, 1, 0LL);
}


void __fastcall ExUISpriteRenderer__OnInit(ExUISpriteRenderer_o *this, const MethodInfo *method)
{
  const MethodInfo *v3; // x1

  UIWidget__OnInit((UIWidget_o *)this, 0LL);
  ExUISpriteRenderer__InitBaseScale(this, v3);
}


void __fastcall ExUISpriteRenderer__OnStart(ExUISpriteRenderer_o *this, const MethodInfo *method)
{
  __int64 v2; // x2
  __int64 v4; // x1
  __int64 v5; // x2
  __int64 v6; // x1
  UnityEngine_Object_o *spriteRenderer; // x20
  __int64 v8; // x1
  Il2CppObject *Component_object; // x0
  int64_t v10; // x2
  int32_t v11; // w3
  System_String_o *v12; // x4
  BattleSetupInfo_o *v13; // x5
  FollowerInfo_o *v14; // x6
  PartyListViewItem_o *v15; // x7
  UnityEngine_Object_o *defaultTexture; // x20
  const MethodInfo *v17; // x1
  const MethodInfo *v18; // x2
  const MethodInfo *v19; // x1

  if ( (byte_4B15674 & 1) == 0 )
  {
    sub_1BCA7E0(&Method_UnityEngine_Component_GetComponent_SpriteRenderer___, method, v2);
    sub_1BCA7E0(&UnityEngine_Object_TypeInfo, v4, v5);
    byte_4B15674 = 1;
  }
  UIWidget__OnStart((UIWidget_o *)this, 0LL);
  spriteRenderer = (UnityEngine_Object_o *)this->fields.spriteRenderer;
  if ( !UnityEngine_Object_TypeInfo->_2.cctor_finished )
    j_il2cpp_runtime_class_init_0(UnityEngine_Object_TypeInfo, v6);
  if ( UnityEngine_Object__op_Equality(spriteRenderer, 0LL, 0LL) )
  {
    Component_object = UnityEngine_Component__GetComponent_object_(
                         (UnityEngine_Component_o *)this,
                         (const MethodInfo_2F09734 *)Method_UnityEngine_Component_GetComponent_SpriteRenderer___);
    this->fields.spriteRenderer = (struct UnityEngine_SpriteRenderer_o *)Component_object;
    sub_1BCA784(
      (PartyOrganizationUtility_o *)&this->fields.spriteRenderer,
      (int64_t)Component_object,
      v10,
      v11,
      v12,
      v13,
      v14,
      v15);
  }
  defaultTexture = (UnityEngine_Object_o *)this->fields.defaultTexture;
  if ( !UnityEngine_Object_TypeInfo->_2.cctor_finished )
    j_il2cpp_runtime_class_init_0(UnityEngine_Object_TypeInfo, v8);
  if ( UnityEngine_Object__op_Inequality(defaultTexture, 0LL, 0LL) )
    ExUISpriteRenderer__SetImage(this, this->fields.defaultTexture, v18);
  ExUISpriteRenderer__InitBaseScale(this, v17);
  ExUISpriteRenderer__ReScaleUpdate(this, v19);
}


void __fastcall ExUISpriteRenderer__ReScaleUpdate(ExUISpriteRenderer_o *this, const MethodInfo *method)
{
  __int64 v2; // x2
  UnityEngine_SpriteRenderer_o *spriteRenderer; // x0
  __int64 v5; // x1
  UnityEngine_Object_o *sprite; // x20
  const MethodInfo *v7; // x1
  int32_t mWidth; // w22
  float x; // s9
  UnityEngine_SpriteRenderer_o *v10; // x21
  int v11; // w0
  float y; // s10
  int32_t mHeight; // w23
  int v14; // w20
  int v15; // w0
  float v16; // s8
  int v17; // w21
  UnityEngine_Vector3_o v18; // 0:s0.4,4:s1.4,8:s2.4

  if ( (byte_4B1567D & 1) == 0 )
  {
    sub_1BCA7E0(&UnityEngine_Object_TypeInfo, method, v2);
    byte_4B1567D = 1;
  }
  if ( this->fields.isInit
    && (this->fields.oldWidth != this->fields.mWidth || this->fields.oldHeight != this->fields.mHeight) )
  {
    spriteRenderer = this->fields.spriteRenderer;
    if ( spriteRenderer )
    {
      sprite = (UnityEngine_Object_o *)UnityEngine_SpriteRenderer__get_sprite(spriteRenderer, 0LL);
      if ( !UnityEngine_Object_TypeInfo->_2.cctor_finished )
        j_il2cpp_runtime_class_init_0(UnityEngine_Object_TypeInfo, v5);
      if ( !UnityEngine_Object__op_Inequality(sprite, 0LL, 0LL) )
        goto LABEL_15;
      ExUISpriteRenderer__InitBaseScale(this, v7);
      spriteRenderer = this->fields.spriteRenderer;
      if ( spriteRenderer )
      {
        spriteRenderer = (UnityEngine_SpriteRenderer_o *)UnityEngine_SpriteRenderer__get_sprite(spriteRenderer, 0LL);
        if ( spriteRenderer )
        {
          spriteRenderer = (UnityEngine_SpriteRenderer_o *)UnityEngine_Sprite__get_texture(
                                                             (UnityEngine_Sprite_o *)spriteRenderer,
                                                             0LL);
          if ( spriteRenderer )
          {
            mWidth = this->fields.mWidth;
            x = this->fields.baseScale.fields.x;
            v10 = spriteRenderer;
            v11 = ((__int64 (__fastcall *)(UnityEngine_SpriteRenderer_o *, const char *))spriteRenderer->klass[1]._1.gc_desc)(
                    spriteRenderer,
                    spriteRenderer->klass[1]._1.name);
            y = this->fields.baseScale.fields.y;
            mHeight = this->fields.mHeight;
            v14 = v11;
            v15 = (*(__int64 (__fastcall **)(UnityEngine_SpriteRenderer_o *, void *))&v10->klass[1]._1.byval_arg.bits)(
                    v10,
                    v10->klass[1]._1.this_arg.data);
            v16 = this->fields.baseScale.fields.y;
            v17 = v15;
            spriteRenderer = (UnityEngine_SpriteRenderer_o *)UnityEngine_Component__get_transform(
                                                               (UnityEngine_Component_o *)this,
                                                               0LL);
            if ( spriteRenderer )
            {
              v18.fields.y = (float)(y * (float)mHeight) / (float)v17;
              v18.fields.x = (float)(x * (float)mWidth) / (float)v14;
              v18.fields.z = v16;
              UnityEngine_Transform__set_localScale((UnityEngine_Transform_o *)spriteRenderer, v18, 0LL);
LABEL_15:
              *(_QWORD *)&this->fields.oldWidth = *(_QWORD *)&this->fields.mWidth;
              return;
            }
          }
        }
      }
    }
    sub_1BCAA3C(spriteRenderer, method);
  }
}


void __fastcall ExUISpriteRenderer__Resize(
        ExUISpriteRenderer_o *this,
        System_String_o *assetName,
        const MethodInfo *method)
{
  __int64 v5; // x1
  __int64 v6; // x2
  __int64 v7; // x1
  __int64 v8; // x2
  __int64 v9; // x1
  ExUISpriteRenderer_c *v10; // x0
  int32_t BACK_WIDTH_21_9; // w1
  __int64 v12; // x1
  ExUISpriteRenderer_c *v13; // x0
  const MethodInfo *v14; // x1
  int32_t width; // w21
  __int64 v16; // x1
  __int64 v17; // x2
  float height; // s1
  ExUISpriteRenderer_c *v19; // x0
  float v20; // s0
  float v21; // s1
  float v22; // s9
  float v23; // s8
  float v24; // s10
  double v25; // d8
  double v26; // d0
  double v27; // d0
  double v28; // d1
  double v29; // d1
  int32_t v30; // w1
  __int64 v31; // x1
  __int64 v32; // x2
  float v33; // s8
  float v34; // s9
  double v35; // d8
  double v36; // d0
  double v37; // d0
  double v38; // d1
  double v39; // d1
  int32_t v40; // w1
  UnityEngine_Transform_o *transform; // x0
  __int64 v42; // x1
  __int64 v43; // x1
  float x; // s8
  float z; // s9
  float BackFSOffsetY; // s0
  int32_t mHeight; // w22
  struct ExUISpriteRenderer_StaticFields *static_fields; // x8
  float v49; // s10
  int32_t BACK_HEIGHT_16_9; // w20
  int32_t BACK_HEIGHT_21_9; // w21
  double iptr; // [xsp+8h] [xbp-58h] BYREF
  UnityEngine_Vector3_o localPosition; // 0:s0.4,4:s1.4,8:s2.4
  UnityEngine_Vector3_o v54; // 0:s0.4,4:s1.4,8:s2.4

  if ( (byte_4B15677 & 1) == 0 )
  {
    sub_1BCA7E0(&CommonUI_TypeInfo, assetName, method);
    sub_1BCA7E0(&ExUISpriteRenderer_TypeInfo, v5, v6);
    sub_1BCA7E0(&FSUtility_TypeInfo, v7, v8);
    byte_4B15677 = 1;
  }
  if ( !FSUtility_TypeInfo->_2.cctor_finished )
    j_il2cpp_runtime_class_init_0(FSUtility_TypeInfo, assetName);
  if ( FSUtility__IsFullScreenBack(assetName, 0LL) )
  {
    v10 = ExUISpriteRenderer_TypeInfo;
    if ( !ExUISpriteRenderer_TypeInfo->_2.cctor_finished )
    {
      j_il2cpp_runtime_class_init_0(ExUISpriteRenderer_TypeInfo, v9);
      v10 = ExUISpriteRenderer_TypeInfo;
    }
    BACK_WIDTH_21_9 = v10->static_fields->BACK_WIDTH_21_9;
LABEL_15:
    UIWidget__set_width((UIWidget_o *)this, BACK_WIDTH_21_9, 0LL);
    UIWidget__set_height((UIWidget_o *)this, ExUISpriteRenderer_TypeInfo->static_fields->BACK_HEIGHT_16_9, 0LL);
    goto LABEL_60;
  }
  if ( !CommonUI_TypeInfo->_2.cctor_finished )
    j_il2cpp_runtime_class_init_0(CommonUI_TypeInfo, v9);
  if ( CommonUI__GetForceObi_16_9(0LL) )
  {
    v13 = ExUISpriteRenderer_TypeInfo;
    if ( !ExUISpriteRenderer_TypeInfo->_2.cctor_finished )
    {
      j_il2cpp_runtime_class_init_0(ExUISpriteRenderer_TypeInfo, v12);
      v13 = ExUISpriteRenderer_TypeInfo;
    }
    BACK_WIDTH_21_9 = v13->static_fields->BACK_WIDTH_16_9;
    goto LABEL_15;
  }
  width = UnityEngine_Screen__get_width(0LL);
  height = (float)UnityEngine_Screen__get_height(0LL);
  v19 = ExUISpriteRenderer_TypeInfo;
  v20 = (float)width / height;
  v21 = fminf(v20, 2.3333);
  if ( v20 < 1.7778 )
    v22 = 1.7778;
  else
    v22 = v21;
  if ( !ExUISpriteRenderer_TypeInfo->_2.cctor_finished )
  {
    j_il2cpp_runtime_class_init_0(ExUISpriteRenderer_TypeInfo, v16);
    v19 = ExUISpriteRenderer_TypeInfo;
  }
  v23 = (float)(v22 * (float)v19->static_fields->BACK_WIDTH_16_9) * 9.0;
  if ( !byte_4B11755 )
  {
    sub_1BCA7E0(&System_Math_TypeInfo, v16, v17);
    byte_4B11755 = 1;
  }
  v24 = v23 * 0.0625;
  if ( !System_Math_TypeInfo->_2.cctor_finished )
    j_il2cpp_runtime_class_init_0(System_Math_TypeInfo, v16);
  v25 = v24;
  v26 = modf(v24, &iptr);
  if ( v24 >= 0.0 )
  {
    if ( v26 != 0.5 )
    {
      v27 = floor(v25 + 0.5);
      goto LABEL_35;
    }
    v27 = iptr;
    v28 = 1.0;
  }
  else
  {
    if ( v26 != -0.5 )
    {
      v27 = ceil(v25 + -0.5);
      goto LABEL_35;
    }
    v27 = iptr;
    v28 = -1.0;
  }
  v29 = v27 + v28;
  if ( ((__int64)v27 & 1) != 0 )
    v27 = v29;
LABEL_35:
  if ( v27 == INFINITY )
    v30 = 0x80000000;
  else
    v30 = (int)v27;
  UIWidget__set_width((UIWidget_o *)this, v30, 0LL);
  v33 = (float)(v22 * (float)ExUISpriteRenderer_TypeInfo->static_fields->BACK_HEIGHT_16_9) * 9.0;
  if ( !byte_4B11755 )
  {
    sub_1BCA7E0(&System_Math_TypeInfo, v31, v32);
    byte_4B11755 = 1;
  }
  v34 = v33 * 0.0625;
  if ( !System_Math_TypeInfo->_2.cctor_finished )
    j_il2cpp_runtime_class_init_0(System_Math_TypeInfo, v31);
  v35 = v34;
  v36 = modf(v34, &iptr);
  if ( v34 >= 0.0 )
  {
    if ( v36 == 0.5 )
    {
      v37 = iptr;
      v38 = 1.0;
LABEL_47:
      v39 = v37 + v38;
      if ( ((__int64)v37 & 1) != 0 )
        v37 = v39;
      goto LABEL_52;
    }
    v37 = floor(v35 + 0.5);
  }
  else
  {
    if ( v36 == -0.5 )
    {
      v37 = iptr;
      v38 = -1.0;
      goto LABEL_47;
    }
    v37 = ceil(v35 + -0.5);
  }
LABEL_52:
  if ( v37 == INFINITY )
    v40 = 0x80000000;
  else
    v40 = (int)v37;
  UIWidget__set_height((UIWidget_o *)this, v40, 0LL);
  transform = UnityEngine_Component__get_transform((UnityEngine_Component_o *)this, 0LL);
  if ( !transform )
    goto LABEL_61;
  localPosition = UnityEngine_Transform__get_localPosition(transform, 0LL);
  x = localPosition.fields.x;
  z = localPosition.fields.z;
  if ( !FSUtility_TypeInfo->_2.cctor_finished )
    j_il2cpp_runtime_class_init_0(FSUtility_TypeInfo, v43);
  BackFSOffsetY = FSUtility__GetBackFSOffsetY(assetName, 0LL);
  mHeight = this->fields.mHeight;
  static_fields = ExUISpriteRenderer_TypeInfo->static_fields;
  v49 = BackFSOffsetY;
  BACK_HEIGHT_16_9 = static_fields->BACK_HEIGHT_16_9;
  BACK_HEIGHT_21_9 = static_fields->BACK_HEIGHT_21_9;
  transform = UnityEngine_Component__get_transform((UnityEngine_Component_o *)this, 0LL);
  if ( !transform )
LABEL_61:
    sub_1BCAA3C(transform, v42);
  v54.fields.y = (float)(v49 * (float)(mHeight - BACK_HEIGHT_16_9)) / (float)(BACK_HEIGHT_21_9 - BACK_HEIGHT_16_9);
  v54.fields.x = x;
  v54.fields.z = z;
  UnityEngine_Transform__set_localPosition(transform, v54, 0LL);
LABEL_60:
  ExUISpriteRenderer__ReScaleUpdate(this, v14);
}


void __fastcall ExUISpriteRenderer__SetAssetImage(
        ExUISpriteRenderer_o *this,
        System_String_o *assetName,
        System_Action_o *callback,
        const MethodInfo *method)
{
  const MethodInfo *v4; // x5

  ExUISpriteRenderer__LoadAssetTexture(this, assetName, 0LL, 0, callback, v4);
}


void __fastcall ExUISpriteRenderer__SetAssetImage2(
        ExUISpriteRenderer_o *this,
        System_String_o *assetName,
        System_String_o *assetLabel,
        System_Action_o *callback,
        const MethodInfo *method)
{
  const MethodInfo *v5; // x5

  ExUISpriteRenderer__LoadAssetTexture(this, assetName, assetLabel, 1, callback, v5);
}


void __fastcall ExUISpriteRenderer__SetAssetImageBackFS(
        ExUISpriteRenderer_o *this,
        System_String_o *assetName,
        System_Action_o *callback,
        const MethodInfo *method)
{
  const MethodInfo *v4; // x5
  const MethodInfo *v7; // x2

  ExUISpriteRenderer__LoadAssetTexture(this, assetName, 0LL, 0, callback, v4);
  ExUISpriteRenderer__Resize(this, assetName, v7);
}


void __fastcall ExUISpriteRenderer__SetAssetImage_38830484(
        ExUISpriteRenderer_o *this,
        System_String_o *assetName,
        System_String_o *assetLabel,
        System_Action_o *callback,
        const MethodInfo *method)
{
  const MethodInfo *v5; // x5

  ExUISpriteRenderer__LoadAssetTexture(this, assetName, assetLabel, 0, callback, v5);
}


void __fastcall ExUISpriteRenderer__SetAssetTexture(ExUISpriteRenderer_o *this, const MethodInfo *method)
{
  __int64 v2; // x2
  __int64 v4; // x1
  __int64 v5; // x2
  __int64 v6; // x1
  __int64 v7; // x2
  __int64 v8; // x1
  __int64 v9; // x2
  UnityEngine_Renderer_o *spriteRenderer; // x0
  _BOOL4 isUseAlphaImage; // w8
  AssetData_o *assetData; // x20
  UnityEngine_Texture_o *v13; // x20
  UnityEngine_Shader_o *shader; // x22
  __int64 v15; // x1
  __int64 v16; // x2
  __int64 v17; // x3
  UnityEngine_Material_o *v18; // x21
  UnityEngine_Texture2D_o *v19; // x20
  int v20; // w22
  int v21; // w0
  UnityEngine_SpriteRenderer_o *v22; // x21
  float v23; // s9
  const MethodInfo *v24; // x1
  float x; // s9
  int32_t mWidth; // w22
  int v27; // w0
  float y; // s10
  int32_t mHeight; // w23
  int v30; // w21
  int v31; // w0
  float v32; // s8
  int v33; // w20
  UnityEngine_Vector2_o pivotOffset; // 0:s4.4,4:s5.4
  UnityEngine_Vector3_o v35; // 0:s0.4,4:s1.4,8:s2.4
  UnityEngine_Rect_o v36; // 0:s0.4,4:s1.4,8:s2.4,12:s3.4

  if ( (byte_4B1567C & 1) == 0 )
  {
    sub_1BCA7E0(&Method_AssetData_GetObject_Texture2D____76836720, method, v2);
    sub_1BCA7E0(&UnityEngine_Material_TypeInfo, v4, v5);
    sub_1BCA7E0(&StringLiteral_16789/*"a"*/, v6, v7);
    sub_1BCA7E0(&StringLiteral_16522/*"_SubTex"*/, v8, v9);
    byte_4B1567C = 1;
  }
  if ( this->fields.isTextureAlphaImage )
  {
    spriteRenderer = (UnityEngine_Renderer_o *)this->fields.spriteRenderer;
    if ( !spriteRenderer )
      goto LABEL_19;
    spriteRenderer = (UnityEngine_Renderer_o *)UnityEngine_Renderer__get_material(spriteRenderer, 0LL);
    if ( !spriteRenderer )
      goto LABEL_19;
    UnityEngine_Material__SetTexture(
      (UnityEngine_Material_o *)spriteRenderer,
      (System_String_o *)StringLiteral_16522/*"_SubTex"*/,
      0LL,
      0LL);
  }
  isUseAlphaImage = this->fields.isUseAlphaImage;
  this->fields.isTextureAlphaImage = isUseAlphaImage;
  if ( isUseAlphaImage )
  {
    assetData = this->fields.assetData;
    spriteRenderer = (UnityEngine_Renderer_o *)System_String__Concat_62401220(
                                                 this->fields.assetLabel,
                                                 (System_String_o *)StringLiteral_16789/*"a"*/,
                                                 0LL);
    if ( assetData )
    {
      spriteRenderer = (UnityEngine_Renderer_o *)AssetData__GetObject_object__49237568(
                                                   assetData,
                                                   (System_String_o *)spriteRenderer,
                                                   (const MethodInfo_2EF4E40 *)Method_AssetData_GetObject_Texture2D____76836720);
      if ( this->fields.spriteRenderer )
      {
        v13 = (UnityEngine_Texture_o *)spriteRenderer;
        spriteRenderer = (UnityEngine_Renderer_o *)UnityEngine_Renderer__get_material(
                                                     (UnityEngine_Renderer_o *)this->fields.spriteRenderer,
                                                     0LL);
        if ( spriteRenderer )
        {
          shader = UnityEngine_Material__get_shader((UnityEngine_Material_o *)spriteRenderer, 0LL);
          v18 = (UnityEngine_Material_o *)sub_1BCAA2C(UnityEngine_Material_TypeInfo, v15, v16, v17);
          UnityEngine_Material___ctor(v18, shader, 0LL);
          if ( v18 )
          {
            UnityEngine_Material__SetTexture(v18, (System_String_o *)StringLiteral_16522/*"_SubTex"*/, v13, 0LL);
            spriteRenderer = (UnityEngine_Renderer_o *)this->fields.spriteRenderer;
            if ( spriteRenderer )
            {
              UnityEngine_Renderer__set_material(spriteRenderer, v18, 0LL);
              goto LABEL_14;
            }
          }
        }
      }
    }
LABEL_19:
    sub_1BCAA3C(spriteRenderer, method);
  }
LABEL_14:
  spriteRenderer = (UnityEngine_Renderer_o *)this->fields.assetData;
  if ( !spriteRenderer )
    goto LABEL_19;
  spriteRenderer = (UnityEngine_Renderer_o *)AssetData__GetObject_object__49237568(
                                               (AssetData_o *)spriteRenderer,
                                               this->fields.assetLabel,
                                               (const MethodInfo_2EF4E40 *)Method_AssetData_GetObject_Texture2D____76836720);
  if ( !spriteRenderer )
    goto LABEL_19;
  v19 = (UnityEngine_Texture2D_o *)spriteRenderer;
  v20 = ((__int64 (__fastcall *)(UnityEngine_Renderer_o *, const char *))spriteRenderer->klass[1]._1.gc_desc)(
          spriteRenderer,
          spriteRenderer->klass[1]._1.name);
  v21 = ((__int64 (__fastcall *)(UnityEngine_Texture2D_o *, Il2CppMethodPointer))v19->klass->vtable._6_get_height.method)(
          v19,
          v19->klass->vtable._7_set_height.methodPtr);
  v22 = this->fields.spriteRenderer;
  v23 = (float)v21;
  pivotOffset = UIWidget__get_pivotOffset((UIWidget_o *)this, 0LL);
  v36.fields.m_XMin = 0.0;
  v36.fields.m_YMin = 0.0;
  v36.fields.m_Width = (float)v20;
  v36.fields.m_Height = v23;
  spriteRenderer = (UnityEngine_Renderer_o *)UnityEngine_Sprite__Create_70184928(v19, v36, pivotOffset, 1.0, 0LL);
  if ( !v22 )
    goto LABEL_19;
  UnityEngine_SpriteRenderer__set_sprite(v22, (UnityEngine_Sprite_o *)spriteRenderer, 0LL);
  ExUISpriteRenderer__InitBaseScale(this, v24);
  x = this->fields.baseScale.fields.x;
  mWidth = this->fields.mWidth;
  v27 = ((__int64 (__fastcall *)(UnityEngine_Texture2D_o *, Il2CppMethodPointer))v19->klass->vtable._4_get_width.method)(
          v19,
          v19->klass->vtable._5_set_width.methodPtr);
  y = this->fields.baseScale.fields.y;
  mHeight = this->fields.mHeight;
  v30 = v27;
  v31 = ((__int64 (__fastcall *)(UnityEngine_Texture2D_o *, Il2CppMethodPointer))v19->klass->vtable._6_get_height.method)(
          v19,
          v19->klass->vtable._7_set_height.methodPtr);
  v32 = this->fields.baseScale.fields.y;
  v33 = v31;
  spriteRenderer = (UnityEngine_Renderer_o *)UnityEngine_Component__get_transform((UnityEngine_Component_o *)this, 0LL);
  if ( !spriteRenderer )
    goto LABEL_19;
  v35.fields.y = (float)(y * (float)mHeight) / (float)v33;
  v35.fields.x = (float)(x * (float)mWidth) / (float)v30;
  v35.fields.z = v32;
  UnityEngine_Transform__set_localScale((UnityEngine_Transform_o *)spriteRenderer, v35, 0LL);
  *(_QWORD *)&this->fields.oldWidth = *(_QWORD *)&this->fields.mWidth;
}


void __fastcall ExUISpriteRenderer__SetImage(
        ExUISpriteRenderer_o *this,
        UnityEngine_Texture2D_o *tex,
        const MethodInfo *method)
{
  AssetData_o *assetData; // x21
  int64_t v6; // x2
  int32_t v7; // w3
  System_String_o *v8; // x4
  BattleSetupInfo_o *v9; // x5
  FollowerInfo_o *v10; // x6
  PartyListViewItem_o *v11; // x7
  System_String_o *loadAssetName; // x21
  int64_t v13; // x2
  int32_t v14; // w3
  System_String_o *v15; // x4
  BattleSetupInfo_o *v16; // x5
  FollowerInfo_o *v17; // x6
  PartyListViewItem_o *v18; // x7
  struct System_Action_o *callbackFunc; // x21
  int64_t v20; // x2
  int32_t v21; // w3
  System_String_o *v22; // x4
  BattleSetupInfo_o *v23; // x5
  FollowerInfo_o *v24; // x6
  PartyListViewItem_o *v25; // x7

  if ( (byte_4B15676 & 1) == 0 )
  {
    sub_1BCA7E0(&AssetManager_TypeInfo, tex, method);
    byte_4B15676 = 1;
  }
  assetData = this->fields.assetData;
  if ( assetData )
  {
    if ( !AssetManager_TypeInfo->_2.cctor_finished )
      j_il2cpp_runtime_class_init_0(AssetManager_TypeInfo, tex);
    AssetManager__releaseAsset_38505704(assetData, 0LL);
    this->fields.assetData = 0LL;
    sub_1BCA784((PartyOrganizationUtility_o *)&this->fields.assetData, 0LL, v6, v7, v8, v9, v10, v11);
  }
  loadAssetName = this->fields.loadAssetName;
  if ( loadAssetName )
  {
    if ( !AssetManager_TypeInfo->_2.cctor_finished )
      j_il2cpp_runtime_class_init_0(AssetManager_TypeInfo, tex);
    AssetManager__releaseAssetStorage(loadAssetName, 0LL);
    this->fields.loadAssetName = 0LL;
    sub_1BCA784((PartyOrganizationUtility_o *)&this->fields.loadAssetName, 0LL, v13, v14, v15, v16, v17, v18);
    callbackFunc = this->fields.callbackFunc;
    this->fields.callbackFunc = 0LL;
    sub_1BCA784((PartyOrganizationUtility_o *)&this->fields.callbackFunc, 0LL, v20, v21, v22, v23, v24, v25);
    if ( callbackFunc )
      ((void (__fastcall *)(struct System_Reflection_MethodInfo_o *, _QWORD))callbackFunc->fields.m_target)(
        callbackFunc->fields.original_method_info,
        *(_QWORD *)&callbackFunc->fields.extra_arg);
  }
  ExUISpriteRenderer__SetTexture(this, tex, method);
}


// local variable allocation has failed, the output may be wrong!
void __fastcall ExUISpriteRenderer__SetSceneMode(
        ExUISpriteRenderer_o *this,
        System_String_o *assetName,
        int32_t sceneMode,
        const MethodInfo *method)
{
  __int64 v7; // x1
  __int64 v8; // x2
  __int64 v9; // x1
  __int64 v10; // x2
  __int64 v11; // x1
  __int64 v12; // x2
  void *Instance; // x0
  __int64 v14; // x1
  __int64 v15; // x1
  UnityEngine_GameObject_o *gameObject; // x0
  int32_t width; // w20
  __int64 v18; // x1
  float height; // s1
  ManagerConfig_c *v20; // x0
  float v21; // s0
  float v22; // s1
  float v23; // s8
  float v24; // s9
  UnityEngine_GameObject_o *v25; // x20
  UnityEngine_GameObject_o *v26; // x0
  float LocalScaleX; // s0
  UnityEngine_GameObject_o *v28; // x20
  UnityEngine_GameObject_o *v29; // x0
  float LocalScaleY; // s0
  __int64 v31; // x1
  ExUISpriteRenderer_c *v32; // x0
  float v33; // s8
  UnityEngine_GameObject_o *v34; // x0
  int32_t v35; // w1
  __int64 v36; // x1
  UIWidget_o *v37; // x19
  UnityEngine_GameObject_o *v38; // x0
  __int64 v39; // x1
  ManagerConfig_c *v40; // x8
  UnityEngine_GameObject_o *v41; // x19
  int v42; // w8
  int v43; // w8

  if ( (byte_4B15678 & 1) == 0 )
  {
    sub_1BCA7E0(&ExUISpriteRenderer_TypeInfo, assetName, *(_QWORD *)&sceneMode);
    sub_1BCA7E0(&FSUtility_TypeInfo, v7, v8);
    sub_1BCA7E0(&ManagerConfig_TypeInfo, v9, v10);
    sub_1BCA7E0(&Method_SingletonMonoBehaviour_ScriptManager__get_Instance__, v11, v12);
    byte_4B15678 = 1;
  }
  Instance = SingletonMonoBehaviour_object___get_Instance((const MethodInfo_37DEB14 *)Method_SingletonMonoBehaviour_ScriptManager__get_Instance__);
  if ( !Instance )
    goto LABEL_40;
  Instance = (void *)*((_QWORD *)Instance + 74);
  if ( !Instance )
    goto LABEL_40;
  Instance = UnityEngine_Component__get_gameObject((UnityEngine_Component_o *)Instance, 0LL);
  if ( !Instance )
    goto LABEL_40;
  UnityEngine_GameObject__SetActive((UnityEngine_GameObject_o *)Instance, 0, 0LL);
  if ( !FSUtility_TypeInfo->_2.cctor_finished )
    j_il2cpp_runtime_class_init_0(FSUtility_TypeInfo, v15);
  if ( FSUtility__IsFullScreenBack(assetName, 0LL) )
  {
    gameObject = UnityEngine_Component__get_gameObject((UnityEngine_Component_o *)this, 0LL);
    GameObjectExtensions__SetLocalPositionY(gameObject, 0.0, 0LL);
    if ( sceneMode == 1 )
    {
      width = UnityEngine_Screen__get_width(0LL);
      height = (float)UnityEngine_Screen__get_height(0LL);
      v20 = ManagerConfig_TypeInfo;
      v21 = (float)width / height;
      v22 = fminf(v21, 2.3333);
      if ( v21 < 1.7778 )
        v23 = 1.7778;
      else
        v23 = v22;
      if ( !ManagerConfig_TypeInfo->_2.cctor_finished )
      {
        j_il2cpp_runtime_class_init_0(ManagerConfig_TypeInfo, v18);
        v20 = ManagerConfig_TypeInfo;
      }
      v24 = (float)v20->static_fields->WIDTH_21_9 / (float)v20->static_fields->HEIGHT;
      v25 = UnityEngine_Component__get_gameObject((UnityEngine_Component_o *)this, 0LL);
      v26 = UnityEngine_Component__get_gameObject((UnityEngine_Component_o *)this, 0LL);
      LocalScaleX = GameObjectExtensions__GetLocalScaleX(v26, 0LL);
      GameObjectExtensions__SetLocalScaleX(v25, LocalScaleX * (float)(v23 / v24), 0LL);
      v28 = UnityEngine_Component__get_gameObject((UnityEngine_Component_o *)this, 0LL);
      v29 = UnityEngine_Component__get_gameObject((UnityEngine_Component_o *)this, 0LL);
      LocalScaleY = GameObjectExtensions__GetLocalScaleY(v29, 0LL);
      GameObjectExtensions__SetLocalScaleY(v28, (float)(v23 / v24) * LocalScaleY, 0LL);
      v32 = ExUISpriteRenderer_TypeInfo;
      if ( !ExUISpriteRenderer_TypeInfo->_2.cctor_finished )
      {
        j_il2cpp_runtime_class_init_0(ExUISpriteRenderer_TypeInfo, v31);
        v32 = ExUISpriteRenderer_TypeInfo;
      }
      v33 = (float)((float)(v24 - v23)
                  / (float)(v24
                          - (float)((float)ManagerConfig_TypeInfo->static_fields->WIDTH
                                  / (float)ManagerConfig_TypeInfo->static_fields->HEIGHT)))
          * (float)(v32->static_fields->BACK_HEIGHT_16_9 - ManagerConfig_TypeInfo->static_fields->HEIGHT);
      v34 = UnityEngine_Component__get_gameObject((UnityEngine_Component_o *)this, 0LL);
      GameObjectExtensions__SetLocalPositionY(v34, v33, 0LL);
      Instance = SingletonMonoBehaviour_object___get_Instance((const MethodInfo_37DEB14 *)Method_SingletonMonoBehaviour_ScriptManager__get_Instance__);
      if ( Instance )
      {
        Instance = (void *)*((_QWORD *)Instance + 74);
        if ( Instance )
        {
          Instance = UnityEngine_Component__get_gameObject((UnityEngine_Component_o *)Instance, 0LL);
          if ( Instance )
          {
            UnityEngine_GameObject__SetActive((UnityEngine_GameObject_o *)Instance, 1, 0LL);
            Instance = SingletonMonoBehaviour_object___get_Instance((const MethodInfo_37DEB14 *)Method_SingletonMonoBehaviour_ScriptManager__get_Instance__);
            if ( Instance )
            {
              Instance = (void *)*((_QWORD *)Instance + 74);
              if ( Instance )
              {
                if ( v33 == INFINITY )
                  v35 = 0;
                else
                  v35 = 2 * (int)v33;
                UIWidget__set_height((UIWidget_o *)Instance, v35, 0LL);
                if ( !FSUtility_TypeInfo->_2.cctor_finished )
                  j_il2cpp_runtime_class_init_0(FSUtility_TypeInfo, v36);
                if ( !FSUtility__IsTablet(0LL) )
                {
                  Instance = SingletonMonoBehaviour_object___get_Instance((const MethodInfo_37DEB14 *)Method_SingletonMonoBehaviour_ScriptManager__get_Instance__);
                  if ( !Instance )
                    goto LABEL_40;
                  v37 = (UIWidget_o *)*((_QWORD *)Instance + 74);
                  Instance = (void *)UnityEngine_Screen__get_width(0LL);
                  if ( !v37 )
                    goto LABEL_40;
                  UIWidget__set_width(v37, (int32_t)Instance, 0LL);
                }
                Instance = SingletonMonoBehaviour_object___get_Instance((const MethodInfo_37DEB14 *)Method_SingletonMonoBehaviour_ScriptManager__get_Instance__);
                if ( Instance )
                {
                  Instance = (void *)*((_QWORD *)Instance + 74);
                  if ( Instance )
                  {
                    v38 = UnityEngine_Component__get_gameObject((UnityEngine_Component_o *)Instance, 0LL);
                    v40 = ManagerConfig_TypeInfo;
                    v41 = v38;
                    if ( !ManagerConfig_TypeInfo->_2.cctor_finished )
                    {
                      j_il2cpp_runtime_class_init_0(ManagerConfig_TypeInfo, v39);
                      v40 = ManagerConfig_TypeInfo;
                    }
                    v42 = v40->static_fields->HEIGHT;
                    if ( v42 <= 0 )
                      v43 = -v42;
                    else
                      v43 = 1 - v42;
                    GameObjectExtensions__SetLocalPositionY(v41, v33 + (float)(v43 >> 1), 0LL);
                    return;
                  }
                }
              }
            }
          }
        }
      }
LABEL_40:
      sub_1BCAA3C(Instance, v14);
    }
  }
}


void __fastcall ExUISpriteRenderer__SetTexture(
        ExUISpriteRenderer_o *this,
        UnityEngine_Texture2D_o *tex,
        const MethodInfo *method)
{
  ExUISpriteRenderer_o *v4; // x19
  int v5; // w22
  int v6; // w0
  UnityEngine_SpriteRenderer_o *spriteRenderer; // x21
  float v8; // s9
  const MethodInfo *v9; // x1
  float x; // s9
  int mWidth; // w22
  int v12; // w0
  float y; // s10
  int mHeight; // w23
  int v15; // w21
  int v16; // w0
  float v17; // s8
  int v18; // w20
  UnityEngine_Vector2_o pivotOffset; // 0:s4.4,4:s5.4
  UnityEngine_Vector3_o v20; // 0:s0.4,4:s1.4,8:s2.4
  UnityEngine_Rect_o v21; // 0:s0.4,4:s1.4,8:s2.4,12:s3.4

  v4 = this;
  if ( (byte_4B1567B & 1) == 0 )
  {
    this = (ExUISpriteRenderer_o *)sub_1BCA7E0(&StringLiteral_16522/*"_SubTex"*/, tex, method);
    byte_4B1567B = 1;
  }
  if ( v4->fields.isTextureAlphaImage )
  {
    this = (ExUISpriteRenderer_o *)v4->fields.spriteRenderer;
    if ( !this
      || (this = (ExUISpriteRenderer_o *)UnityEngine_Renderer__get_material((UnityEngine_Renderer_o *)this, 0LL)) == 0LL )
    {
LABEL_11:
      sub_1BCAA3C(this, tex);
    }
    UnityEngine_Material__SetTexture((UnityEngine_Material_o *)this, (System_String_o *)StringLiteral_16522/*"_SubTex"*/, 0LL, 0LL);
  }
  v4->fields.isTextureAlphaImage = 0;
  if ( !tex )
    goto LABEL_11;
  v5 = ((__int64 (__fastcall *)(UnityEngine_Texture2D_o *, Il2CppMethodPointer, const MethodInfo *))tex->klass->vtable._4_get_width.method)(
         tex,
         tex->klass->vtable._5_set_width.methodPtr,
         method);
  v6 = ((__int64 (__fastcall *)(UnityEngine_Texture2D_o *, Il2CppMethodPointer))tex->klass->vtable._6_get_height.method)(
         tex,
         tex->klass->vtable._7_set_height.methodPtr);
  spriteRenderer = v4->fields.spriteRenderer;
  v8 = (float)v6;
  pivotOffset = UIWidget__get_pivotOffset((UIWidget_o *)v4, 0LL);
  v21.fields.m_XMin = 0.0;
  v21.fields.m_YMin = 0.0;
  v21.fields.m_Width = (float)v5;
  v21.fields.m_Height = v8;
  this = (ExUISpriteRenderer_o *)UnityEngine_Sprite__Create_70184928(tex, v21, pivotOffset, 1.0, 0LL);
  if ( !spriteRenderer )
    goto LABEL_11;
  UnityEngine_SpriteRenderer__set_sprite(spriteRenderer, (UnityEngine_Sprite_o *)this, 0LL);
  ExUISpriteRenderer__InitBaseScale(v4, v9);
  x = v4->fields.baseScale.fields.x;
  mWidth = v4->fields.mWidth;
  v12 = ((__int64 (__fastcall *)(UnityEngine_Texture2D_o *, Il2CppMethodPointer))tex->klass->vtable._4_get_width.method)(
          tex,
          tex->klass->vtable._5_set_width.methodPtr);
  y = v4->fields.baseScale.fields.y;
  mHeight = v4->fields.mHeight;
  v15 = v12;
  v16 = ((__int64 (__fastcall *)(UnityEngine_Texture2D_o *, Il2CppMethodPointer))tex->klass->vtable._6_get_height.method)(
          tex,
          tex->klass->vtable._7_set_height.methodPtr);
  v17 = v4->fields.baseScale.fields.y;
  v18 = v16;
  this = (ExUISpriteRenderer_o *)UnityEngine_Component__get_transform((UnityEngine_Component_o *)v4, 0LL);
  if ( !this )
    goto LABEL_11;
  v20.fields.y = (float)(y * (float)mHeight) / (float)v18;
  v20.fields.x = (float)(x * (float)mWidth) / (float)v15;
  v20.fields.z = v17;
  UnityEngine_Transform__set_localScale((UnityEngine_Transform_o *)this, v20, 0LL);
  *(_QWORD *)&v4->fields.oldWidth = *(_QWORD *)&v4->fields.mWidth;
}


// local variable allocation has failed, the output may be wrong!
void __fastcall ExUISpriteRenderer__SetTweenColor(
        ExUISpriteRenderer_o *this,
        UnityEngine_Color_o c,
        const MethodInfo *method)
{
  float a; // s8
  float b; // s9
  float g; // s10
  float r; // s11
  __int64 v8; // x1
  UnityEngine_Renderer_o *spriteRenderer; // x0
  UnityEngine_Color_o v10; // 0:s0.4,4:s1.4,8:s2.4,12:s3.4

  a = c.fields.a;
  b = c.fields.b;
  g = c.fields.g;
  r = c.fields.r;
  UIWidget__set_color((UIWidget_o *)this, c, 0LL);
  spriteRenderer = (UnityEngine_Renderer_o *)this->fields.spriteRenderer;
  if ( !spriteRenderer
    || (UnityEngine_Renderer__set_enabled(spriteRenderer, a > 0.0, 0LL),
        (spriteRenderer = (UnityEngine_Renderer_o *)this->fields.spriteRenderer) == 0LL) )
  {
    sub_1BCAA3C(spriteRenderer, v8);
  }
  v10.fields.b = b;
  v10.fields.a = a;
  v10.fields.r = r;
  v10.fields.g = g;
  UnityEngine_SpriteRenderer__set_color((UnityEngine_SpriteRenderer_o *)spriteRenderer, v10, 0LL);
}