void __fastcall MapTouchDetector___ctor(MapTouchDetector_o *this, const MethodInfo *method)
{
  System_Object___ctor((Il2CppObject *)this, 0LL);
}


void __fastcall MapTouchDetector__DetectTouchLocally(MapTouchDetector_o *this, const MethodInfo *method)
{
  char v3; // w0
  MapTouchDetector_c *klass; // x8
  struct TouchDetectorBase_TouchInfo_o *v5; // x0
  int32_t v6; // w2
  int32_t v7; // w3
  int32_t v8; // w2
  int32_t v9; // w3
  struct TouchDetectorBase_TouchInfo_o *StartTouchInfo_k__BackingField; // x1
  struct TouchDetectorBase_TouchInfo_o **p_CurrentTouchInfo_k__BackingField; // x0
  const MethodInfo *v12; // x1
  MapTouchDetector_Fields *p_fields; // x21
  TouchDetectorBase_TouchInfo_o *v14; // x0
  const MethodInfo *v15; // x1
  struct TouchDetectorBase_TouchInfo_o *v16; // x20
  __int64 TouchCount; // x0
  const MethodInfo *v18; // x1
  int32_t v19; // w2
  int32_t v20; // w3
  int32_t v21; // w2
  int32_t v22; // w3
  struct TouchDetectorBase_TouchInfo_o *CurrentTouchInfo_k__BackingField; // x1

  v3 = ((__int64 (__fastcall *)(MapTouchDetector_o *, Il2CppMethodPointer))this->klass->vtable._5_IsTouchStart.method)(
         this,
         this->klass->vtable._6_IsTouchKeep.methodPtr);
  klass = this->klass;
  if ( (v3 & 1) != 0 )
  {
    v5 = (struct TouchDetectorBase_TouchInfo_o *)((__int64 (__fastcall *)(MapTouchDetector_o *, void *))klass->vtable._7_GetCurrentTouchInfo.method)(
                                                   this,
                                                   klass[1]._1.image);
    this->fields._StartTouchInfo_k__BackingField = v5;
    sub_1B6406C((ServantStatusBattleListViewItem_o *)&this->fields, (int32_t)v5, v6, v7);
    StartTouchInfo_k__BackingField = this->fields._StartTouchInfo_k__BackingField;
    this->fields._CurrentTouchInfo_k__BackingField = StartTouchInfo_k__BackingField;
    p_CurrentTouchInfo_k__BackingField = &this->fields._CurrentTouchInfo_k__BackingField;
LABEL_3:
    sub_1B6406C(
      (ServantStatusBattleListViewItem_o *)p_CurrentTouchInfo_k__BackingField,
      (int32_t)StartTouchInfo_k__BackingField,
      v8,
      v9);
    return;
  }
  if ( (((__int64 (__fastcall *)(MapTouchDetector_o *, Il2CppMethodPointer))klass->vtable._6_IsTouchKeep.method)(
          this,
          klass->vtable._7_GetCurrentTouchInfo.methodPtr) & 1) != 0
    && (p_fields = &this->fields, this->fields._StartTouchInfo_k__BackingField) )
  {
    v14 = (TouchDetectorBase_TouchInfo_o *)((__int64 (__fastcall *)(MapTouchDetector_o *, void *))this->klass->vtable._7_GetCurrentTouchInfo.method)(
                                             this,
                                             this->klass[1]._1.image);
    if ( v14 )
    {
      v16 = v14;
      TouchCount = TouchDetectorBase_TouchInfo__get_TouchCount(v14, v15);
      if ( !p_fields->_StartTouchInfo_k__BackingField )
        sub_1B64324(TouchCount);
      if ( (_DWORD)TouchCount != TouchDetectorBase_TouchInfo__get_TouchCount(
                                   p_fields->_StartTouchInfo_k__BackingField,
                                   v18) )
      {
        this->fields._StartTouchInfo_k__BackingField = v16;
        sub_1B6406C((ServantStatusBattleListViewItem_o *)&this->fields, (int32_t)v16, v19, v20);
        this->fields._CurrentTouchInfo_k__BackingField = 0LL;
        sub_1B6406C((ServantStatusBattleListViewItem_o *)&this->fields._CurrentTouchInfo_k__BackingField, 0, v21, v22);
      }
      CurrentTouchInfo_k__BackingField = this->fields._CurrentTouchInfo_k__BackingField;
      this->fields._PrevTouchInfo_k__BackingField = CurrentTouchInfo_k__BackingField;
      sub_1B6406C(
        (ServantStatusBattleListViewItem_o *)&this->fields._PrevTouchInfo_k__BackingField,
        (int32_t)CurrentTouchInfo_k__BackingField,
        v19,
        v20);
      p_CurrentTouchInfo_k__BackingField = &this->fields._CurrentTouchInfo_k__BackingField;
      LODWORD(StartTouchInfo_k__BackingField) = (_DWORD)v16;
      this->fields._CurrentTouchInfo_k__BackingField = v16;
      goto LABEL_3;
    }
  }
  else
  {
    TouchDetectorBase__ClearTouchInfo((TouchDetectorBase_o *)this, v12);
  }
}


System_Collections_Generic_List_Touch__o *__fastcall MapTouchDetector__GetAvaliableTouchList(
        MapTouchDetector_o *this,
        const MethodInfo *method)
{
  __int64 v2; // x2
  __int64 v3; // x1
  __int64 v4; // x1
  __int64 v5; // x1
  __int64 v6; // x1
  System_Collections_Generic_List_Touch__o *v7; // x19
  int32_t v8; // w20
  __int64 v9; // x1
  CTouch_c *v10; // x0
  System_Collections_Generic_List_int__o *ignoreFingerIDList; // x22
  void *fingerId; // x0
  _QWORD *v13; // x22
  struct UnityEngine_Touch_array *items; // x8
  __int64 size; // x9
  const MethodInfo_34ECBD0 *v16; // x22
  char v18[72]; // [xsp+8h] [xbp-188h] BYREF
  UnityEngine_Touch_o dest; // [xsp+50h] [xbp-140h] BYREF
  char v20[72]; // [xsp+A0h] [xbp-F0h] BYREF
  UnityEngine_Touch_o src; // [xsp+E8h] [xbp-A8h] BYREF

  if ( (byte_49F900E & 1) == 0 )
  {
    sub_1B640C8(&CTouch_TypeInfo, method);
    sub_1B640C8(&Method_System_Collections_Generic_List_Touch__Add__, v3);
    sub_1B640C8(&Method_System_Collections_Generic_List_int__Contains__, v4);
    sub_1B640C8(&Method_System_Collections_Generic_List_Touch___ctor__, v5);
    sub_1B640C8(&System_Collections_Generic_List_Touch__TypeInfo, v6);
    byte_49F900E = 1;
  }
  memset(&dest, 0, sizeof(dest));
  v7 = (System_Collections_Generic_List_Touch__o *)sub_1B64314(
                                                     System_Collections_Generic_List_Touch__TypeInfo,
                                                     method,
                                                     v2);
  System_Collections_Generic_List_Touch____ctor(
    v7,
    (const MethodInfo_34EC2B4 *)Method_System_Collections_Generic_List_Touch___ctor__);
  if ( UnityEngine_Input__get_touchCount(0LL) >= 1 )
  {
    v8 = 0;
    while ( 1 )
    {
      UnityEngine_Input__GetTouch(&src, v8, 0LL);
      memcpy(&dest, &src, sizeof(dest));
      if ( !CTouch_TypeInfo->_2.cctor_finished )
        j_il2cpp_runtime_class_init_0(CTouch_TypeInfo);
      if ( !byte_49F9043 )
      {
        sub_1B640C8(&CTouch_TypeInfo, v9);
        byte_49F9043 = 1;
      }
      v10 = CTouch_TypeInfo;
      if ( !CTouch_TypeInfo->_2.cctor_finished )
      {
        j_il2cpp_runtime_class_init_0(CTouch_TypeInfo);
        v10 = CTouch_TypeInfo;
      }
      ignoreFingerIDList = v10->static_fields->ignoreFingerIDList;
      fingerId = (void *)UnityEngine_Touch__get_fingerId(&dest, 0LL);
      if ( !ignoreFingerIDList )
        break;
      if ( !System_Collections_Generic_List_int___Contains(
              ignoreFingerIDList,
              (int32_t)fingerId,
              (const MethodInfo_3490254 *)Method_System_Collections_Generic_List_int__Contains__) )
      {
        fingerId = memcpy(v18, &dest, 0x44u);
        if ( !v7 )
          break;
        v13 = Method_System_Collections_Generic_List_Touch__Add__;
        fingerId = memcpy(v20, v18, 0x44u);
        items = v7->fields._items;
        ++v7->fields._version;
        if ( !items )
          break;
        size = v7->fields._size;
        if ( (unsigned int)size >= items->max_length )
        {
          v16 = *(const MethodInfo_34ECBD0 **)(*(_QWORD *)(v13[4] + 192LL) + 112LL);
          memcpy(&src, v20, sizeof(src));
          System_Collections_Generic_List_Touch___AddWithResize(v7, &src, v16);
        }
        else
        {
          v7->fields._size = size + 1;
          memcpy(&items->m_Items[size].fields.m_Position, v20, 0x44u);
        }
      }
      if ( ++v8 >= UnityEngine_Input__get_touchCount(0LL) )
        return v7;
    }
    sub_1B64324(fingerId);
  }
  return v7;
}


// local variable allocation has failed, the output may be wrong!
TouchDetectorBase_TouchInfo_o *__fastcall MapTouchDetector__GetCurrentTouchInfo(
        MapTouchDetector_o *this,
        const MethodInfo *method)
{
  __int64 v2; // x1
  __int64 v3; // x1
  __int64 v4; // x1
  __int64 v5; // x1
  __int64 v6; // x1
  __int64 v7; // x1
  __int64 v8; // x1
  __int64 v9; // x1
  __int64 v10; // x1
  __int64 v11; // x1
  __int64 v12; // x1
  __int64 v13; // x1
  System_Collections_Generic_List_Touch__o *AvaliableTouchList; // x0
  System_Collections_Generic_List_Touch__o *v15; // x20
  UnityEngine_Vector2_o position; // kr00_8
  UnityEngine_Vector2_o ScreenPosition_46287392; // kr08_8
  __int64 v18; // x1
  __int64 v19; // x2
  System_Collections_Generic_List_Vector2__o *v20; // x19
  UnityEngine_Vector2_o v21; // kr10_8
  UnityEngine_Vector2_o v22; // x0
  UnityEngine_Vector2_o v23; // kr18_8
  struct UnityEngine_Vector2_array *items; // x8
  _QWORD *v25; // x9
  __int64 size; // x10
  UnityEngine_Vector2_array *v27; // x20
  __int64 v28; // x1
  __int64 v29; // x2
  TouchDetectorBase_TouchInfo_o *v30; // x19
  const MethodInfo *v31; // x2
  UnityEngine_Touch_o src; // [xsp+8h] [xbp-1D8h] BYREF
  UnityEngine_Touch_o v34; // [xsp+60h] [xbp-180h] BYREF
  char v35[80]; // [xsp+B0h] [xbp-130h] BYREF
  __int64 v36; // [xsp+100h] [xbp-E0h]
  UnityEngine_Touch_o dest; // [xsp+110h] [xbp-D0h] BYREF

  if ( (byte_49F900D & 1) == 0 )
  {
    sub_1B640C8(&CTouch_TypeInfo, method);
    sub_1B640C8(&Method_System_Collections_Generic_List_Enumerator_Touch__Dispose__, v2);
    sub_1B640C8(&Method_System_Collections_Generic_List_Enumerator_Touch__MoveNext__, v3);
    sub_1B640C8(&Method_System_Collections_Generic_List_Enumerator_Touch__get_Current__, v4);
    sub_1B640C8(&Method_System_Collections_Generic_List_Vector2__Add__, v5);
    sub_1B640C8(&Method_System_Collections_Generic_List_Touch__GetEnumerator__, v6);
    sub_1B640C8(&Method_System_Collections_Generic_List_Touch__RemoveAt__, v7);
    sub_1B640C8(&Method_System_Collections_Generic_List_Vector2__ToArray__, v8);
    sub_1B640C8(&Method_System_Collections_Generic_List_Vector2___ctor__, v9);
    sub_1B640C8(&Method_System_Collections_Generic_List_Touch__get_Count__, v10);
    sub_1B640C8(&Method_System_Collections_Generic_List_Touch__get_Item__, v11);
    sub_1B640C8(&System_Collections_Generic_List_Vector2__TypeInfo, v12);
    this = (MapTouchDetector_o *)sub_1B640C8(&TouchDetectorBase_TouchInfo_TypeInfo, v13);
    byte_49F900D = 1;
  }
  v36 = 0LL;
  memset(&dest, 0, sizeof(dest));
  memset(v35, 0, sizeof(v35));
  memset(&v34, 0, sizeof(v34));
  AvaliableTouchList = MapTouchDetector__GetAvaliableTouchList(this, method);
  if ( !AvaliableTouchList )
    goto LABEL_22;
  v15 = AvaliableTouchList;
  if ( AvaliableTouchList->fields._size < 1 )
    return 0LL;
  System_Collections_Generic_List_Touch___get_Item(
    &src,
    AvaliableTouchList,
    0,
    (const MethodInfo_34EC818 *)Method_System_Collections_Generic_List_Touch__get_Item__);
  memcpy(&dest, &src, sizeof(dest));
  position = UnityEngine_Touch__get_position(&dest, 0LL);
  if ( !CTouch_TypeInfo->_2.cctor_finished )
    j_il2cpp_runtime_class_init_0(CTouch_TypeInfo);
  ScreenPosition_46287392 = CTouch__getScreenPosition_46287392(position, 0LL);
  System_Collections_Generic_List_Touch___RemoveAt(
    v15,
    0,
    (const MethodInfo_34EE9C4 *)Method_System_Collections_Generic_List_Touch__RemoveAt__);
  v20 = (System_Collections_Generic_List_Vector2__o *)sub_1B64314(
                                                        System_Collections_Generic_List_Vector2__TypeInfo,
                                                        v18,
                                                        v19);
  System_Collections_Generic_List_Vector2____ctor(
    v20,
    (const MethodInfo_3502044 *)Method_System_Collections_Generic_List_Vector2___ctor__);
  System_Collections_Generic_List_Touch___GetEnumerator(
    (System_Collections_Generic_List_Enumerator_T__o *)&src,
    v15,
    (const MethodInfo_34EDAE4 *)Method_System_Collections_Generic_List_Touch__GetEnumerator__);
  memcpy(v35, &src, 0x58u);
  while ( System_Collections_Generic_List_Enumerator_Touch___MoveNext(
            (System_Collections_Generic_List_Enumerator_T__o *)v35,
            (const MethodInfo_322EF7C *)Method_System_Collections_Generic_List_Enumerator_Touch__MoveNext__) )
  {
    memcpy(&v34, &v35[16], sizeof(v34));
    v21 = UnityEngine_Touch__get_position(&v34, 0LL);
    if ( !CTouch_TypeInfo->_2.cctor_finished )
      j_il2cpp_runtime_class_init_0(CTouch_TypeInfo);
    v23 = CTouch__getScreenPosition_46287392(v21, 0LL);
    if ( !v20 )
      ((void (__fastcall __noreturn *)(_QWORD))sub_1B64324)(v22);
    items = v20->fields._items;
    v25 = Method_System_Collections_Generic_List_Vector2__Add__;
    ++v20->fields._version;
    if ( !items )
      ((void (__fastcall __noreturn *)(_QWORD))sub_1B64324)(v22);
    size = v20->fields._size;
    if ( (unsigned int)size >= items->max_length )
    {
      System_Collections_Generic_List_Vector2___AddWithResize(
        v20,
        v23,
        *(const MethodInfo_35028AC **)(*(_QWORD *)(v25[4] + 192LL) + 112LL));
    }
    else
    {
      v20->fields._size = size + 1;
      *(UnityEngine_Vector2_o *)&items->m_Items[size].fields.y = v23;
    }
  }
  System_Collections_Generic_List_Enumerator_Touch___Dispose(
    (System_Collections_Generic_List_Enumerator_T__o *)v35,
    (const MethodInfo_322EF78 *)Method_System_Collections_Generic_List_Enumerator_Touch__Dispose__);
  if ( !v20 )
LABEL_22:
    sub_1B64324(AvaliableTouchList);
  v27 = System_Collections_Generic_List_Vector2___ToArray(
          v20,
          (const MethodInfo_350446C *)Method_System_Collections_Generic_List_Vector2__ToArray__);
  v30 = (TouchDetectorBase_TouchInfo_o *)sub_1B64314(TouchDetectorBase_TouchInfo_TypeInfo, v28, v29);
  TouchDetectorBase_TouchInfo___ctor(v30, ScreenPosition_46287392, v27, v31);
  return v30;
}


bool __fastcall MapTouchDetector__IsTouchKeep(MapTouchDetector_o *this, const MethodInfo *method)
{
  __int64 v2; // x1
  __int64 v3; // x1
  __int64 v4; // x1
  System_Collections_Generic_List_Touch__o *AvaliableTouchList; // x0
  char v6; // w19
  int v7; // w21
  System_Collections_Generic_List_Enumerator_T__o src[3]; // [xsp+8h] [xbp-138h] BYREF
  UnityEngine_Touch_o v10; // [xsp+60h] [xbp-E0h] BYREF
  char dest[80]; // [xsp+B0h] [xbp-90h] BYREF
  __int64 v12; // [xsp+100h] [xbp-40h]

  if ( (byte_49F900C & 1) == 0 )
  {
    sub_1B640C8(&Method_System_Collections_Generic_List_Enumerator_Touch__Dispose__, method);
    sub_1B640C8(&Method_System_Collections_Generic_List_Enumerator_Touch__MoveNext__, v2);
    sub_1B640C8(&Method_System_Collections_Generic_List_Enumerator_Touch__get_Current__, v3);
    this = (MapTouchDetector_o *)sub_1B640C8(&Method_System_Collections_Generic_List_Touch__GetEnumerator__, v4);
    byte_49F900C = 1;
  }
  v12 = 0LL;
  memset(dest, 0, sizeof(dest));
  memset(&v10, 0, sizeof(v10));
  AvaliableTouchList = MapTouchDetector__GetAvaliableTouchList(this, method);
  if ( !AvaliableTouchList )
    sub_1B64324(0LL);
  System_Collections_Generic_List_Touch___GetEnumerator(
    src,
    AvaliableTouchList,
    (const MethodInfo_34EDAE4 *)Method_System_Collections_Generic_List_Touch__GetEnumerator__);
  memcpy(dest, src, 0x58u);
  while ( System_Collections_Generic_List_Enumerator_Touch___MoveNext(
            (System_Collections_Generic_List_Enumerator_T__o *)dest,
            (const MethodInfo_322EF7C *)Method_System_Collections_Generic_List_Enumerator_Touch__MoveNext__) )
  {
    memcpy(&v10, &dest[16], sizeof(v10));
    if ( UnityEngine_Touch__get_phase(&v10, 0LL) == 1 || UnityEngine_Touch__get_phase(&v10, 0LL) == 2 )
    {
      v6 = 1;
      v7 = 5;
      goto LABEL_10;
    }
  }
  v6 = 0;
  v7 = 6;
LABEL_10:
  System_Collections_Generic_List_Enumerator_Touch___Dispose(
    (System_Collections_Generic_List_Enumerator_T__o *)dest,
    (const MethodInfo_322EF78 *)Method_System_Collections_Generic_List_Enumerator_Touch__Dispose__);
  return v6 & (v7 == 5);
}


bool __fastcall MapTouchDetector__IsTouchStart(MapTouchDetector_o *this, const MethodInfo *method)
{
  __int64 v3; // x1
  __int64 v4; // x1
  __int64 v5; // x1
  const MethodInfo *v6; // x1
  char v7; // w8
  bool result; // w0
  System_Collections_Generic_List_Touch__o *AvaliableTouchList; // x0
  bool v10; // w19
  int v11; // w20
  System_Collections_Generic_List_Enumerator_T__o src[3]; // [xsp+8h] [xbp-138h] BYREF
  UnityEngine_Touch_o v13; // [xsp+60h] [xbp-E0h] BYREF
  char dest[80]; // [xsp+B0h] [xbp-90h] BYREF
  __int64 v15; // [xsp+100h] [xbp-40h]

  if ( (byte_49F900B & 1) == 0 )
  {
    sub_1B640C8(&Method_System_Collections_Generic_List_Enumerator_Touch__Dispose__, method);
    sub_1B640C8(&Method_System_Collections_Generic_List_Enumerator_Touch__MoveNext__, v3);
    sub_1B640C8(&Method_System_Collections_Generic_List_Enumerator_Touch__get_Current__, v4);
    sub_1B640C8(&Method_System_Collections_Generic_List_Touch__GetEnumerator__, v5);
    byte_49F900B = 1;
  }
  v15 = 0LL;
  memset(dest, 0, sizeof(dest));
  memset(&v13, 0, sizeof(v13));
  v7 = ((__int64 (__fastcall *)(MapTouchDetector_o *, Il2CppMethodPointer))this->klass->vtable._6_IsTouchKeep.method)(
         this,
         this->klass->vtable._7_GetCurrentTouchInfo.methodPtr);
  result = 0;
  if ( (v7 & 1) == 0 )
  {
    AvaliableTouchList = MapTouchDetector__GetAvaliableTouchList(0LL, v6);
    if ( !AvaliableTouchList )
      sub_1B64324(0LL);
    System_Collections_Generic_List_Touch___GetEnumerator(
      src,
      AvaliableTouchList,
      (const MethodInfo_34EDAE4 *)Method_System_Collections_Generic_List_Touch__GetEnumerator__);
    memcpy(dest, src, 0x58u);
    while ( 1 )
    {
      v10 = System_Collections_Generic_List_Enumerator_Touch___MoveNext(
              (System_Collections_Generic_List_Enumerator_T__o *)dest,
              (const MethodInfo_322EF7C *)Method_System_Collections_Generic_List_Enumerator_Touch__MoveNext__);
      if ( !v10 )
        break;
      memcpy(&v13, &dest[16], sizeof(v13));
      if ( !UnityEngine_Touch__get_phase(&v13, 0LL) )
      {
        v11 = 5;
        goto LABEL_10;
      }
    }
    v11 = 6;
LABEL_10:
    System_Collections_Generic_List_Enumerator_Touch___Dispose(
      (System_Collections_Generic_List_Enumerator_T__o *)dest,
      (const MethodInfo_322EF78 *)Method_System_Collections_Generic_List_Enumerator_Touch__Dispose__);
    return v10 && v11 == 5;
  }
  return result;
}