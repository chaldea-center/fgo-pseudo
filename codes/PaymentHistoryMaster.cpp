void __fastcall PaymentHistoryMaster___ctor(PaymentHistoryMaster_o *this, const MethodInfo *method)
{
  if ( (byte_4A5B571 & 1) == 0 )
  {
    sub_1B885B0(&Method_DataMasterBase_PaymentHistoryMaster__PaymentHistoryEntity__string___ctor__);
    byte_4A5B571 = 1;
  }
  DataMasterBase_object__object__object____ctor(
    (DataMasterBase_TMaster__TEntity__PKType__o *)this,
    498,
    (const MethodInfo_311DC4C *)Method_DataMasterBase_PaymentHistoryMaster__PaymentHistoryEntity__string___ctor__);
}


System_Collections_Generic_List_PaymentHistoryEntity__o *__fastcall PaymentHistoryMaster__GetEntityList(
        PaymentHistoryMaster_o *this,
        const MethodInfo *method)
{
  PaymentHistoryMaster___c_c *v3; // x0
  System_Collections_ObjectModel_ObservableCollection_T__o *list; // x19
  System_Converter_TInput__TOutput__o *_9__1_0; // x20
  Il2CppObject *v6; // x21
  struct PaymentHistoryMaster___c_StaticFields *static_fields; // x0
  int32_t v8; // w2
  int32_t v9; // w3
  System_Collections_Generic_List_TOutput__o *v10; // x0
  __int64 v11; // x1
  PaymentHistoryMaster___c_c *v12; // x8
  System_Collections_Generic_List_object__o *v13; // x19
  System_Comparison_T__o *_9__1_1; // x20
  Il2CppObject *v15; // x21
  struct PaymentHistoryMaster___c_StaticFields *v16; // x0
  int32_t v17; // w2
  int32_t v18; // w3

  if ( (byte_4A5B572 & 1) == 0 )
  {
    sub_1B885B0(&System_Comparison_PaymentHistoryEntity__TypeInfo);
    sub_1B885B0(&System_Converter_DataEntityBase__PaymentHistoryEntity__TypeInfo);
    sub_1B885B0(&Method_System_Collections_Generic_List_PaymentHistoryEntity__Sort__);
    sub_1B885B0(&Method_ObservableCollectionExtensions_ConvertAll_DataEntityBase__PaymentHistoryEntity___);
    sub_1B885B0(&Method_PaymentHistoryMaster___c__GetEntityList_b__1_0__);
    sub_1B885B0(&Method_PaymentHistoryMaster___c__GetEntityList_b__1_1__);
    sub_1B885B0(&PaymentHistoryMaster___c_TypeInfo);
    byte_4A5B572 = 1;
  }
  v3 = PaymentHistoryMaster___c_TypeInfo;
  list = (System_Collections_ObjectModel_ObservableCollection_T__o *)this->fields.list;
  if ( !PaymentHistoryMaster___c_TypeInfo->_2.cctor_finished )
  {
    j_il2cpp_runtime_class_init_0(PaymentHistoryMaster___c_TypeInfo);
    v3 = PaymentHistoryMaster___c_TypeInfo;
  }
  _9__1_0 = (System_Converter_TInput__TOutput__o *)v3->static_fields->__9__1_0;
  if ( !_9__1_0 )
  {
    if ( !v3->_2.cctor_finished )
    {
      j_il2cpp_runtime_class_init_0(v3);
      v3 = PaymentHistoryMaster___c_TypeInfo;
    }
    v6 = (Il2CppObject *)v3->static_fields->__9;
    _9__1_0 = (System_Converter_TInput__TOutput__o *)sub_1B887FC(System_Converter_DataEntityBase__PaymentHistoryEntity__TypeInfo);
    System_Converter_object__object____ctor(_9__1_0, v6, Method_PaymentHistoryMaster___c__GetEntityList_b__1_0__, 0LL);
    static_fields = PaymentHistoryMaster___c_TypeInfo->static_fields;
    static_fields->__9__1_0 = (struct System_Converter_DataEntityBase__PaymentHistoryEntity__o *)_9__1_0;
    sub_1B88554((ServantStatusBattleListViewItem_o *)&static_fields->__9__1_0, (int32_t)_9__1_0, v8, v9);
  }
  v10 = ObservableCollectionExtensions__ConvertAll_object__object_(
          list,
          (System_Converter_T__TOutput__o *)_9__1_0,
          (const MethodInfo_2F042F8 *)Method_ObservableCollectionExtensions_ConvertAll_DataEntityBase__PaymentHistoryEntity___);
  v12 = PaymentHistoryMaster___c_TypeInfo;
  v13 = (System_Collections_Generic_List_object__o *)v10;
  if ( !PaymentHistoryMaster___c_TypeInfo->_2.cctor_finished )
  {
    j_il2cpp_runtime_class_init_0(PaymentHistoryMaster___c_TypeInfo);
    v12 = PaymentHistoryMaster___c_TypeInfo;
  }
  _9__1_1 = (System_Comparison_T__o *)v12->static_fields->__9__1_1;
  if ( !_9__1_1 )
  {
    if ( !v12->_2.cctor_finished )
    {
      j_il2cpp_runtime_class_init_0(v12);
      v12 = PaymentHistoryMaster___c_TypeInfo;
    }
    v15 = (Il2CppObject *)v12->static_fields->__9;
    _9__1_1 = (System_Comparison_T__o *)sub_1B887FC(System_Comparison_PaymentHistoryEntity__TypeInfo);
    System_Comparison_object____ctor(_9__1_1, v15, Method_PaymentHistoryMaster___c__GetEntityList_b__1_1__, 0LL);
    v16 = PaymentHistoryMaster___c_TypeInfo->static_fields;
    v16->__9__1_1 = (struct System_Comparison_PaymentHistoryEntity__o *)_9__1_1;
    sub_1B88554((ServantStatusBattleListViewItem_o *)&v16->__9__1_1, (int32_t)_9__1_1, v17, v18);
  }
  if ( !v13 )
    sub_1B8880C(v10, v11);
  System_Collections_Generic_List_object___Sort_55571192(
    v13,
    _9__1_1,
    (const MethodInfo_34FF2F8 *)Method_System_Collections_Generic_List_PaymentHistoryEntity__Sort__);
  return (System_Collections_Generic_List_PaymentHistoryEntity__o *)v13;
}


System_Collections_Generic_IEnumerator_PaymentHistoryEntity__o *__fastcall PaymentHistoryMaster__GetEnumerator(
        PaymentHistoryMaster_o *this,
        const MethodInfo *method)
{
  System_Collections_Generic_List_object__o *EntityList; // x0
  __int64 v4; // x1
  __int64 v5; // x2
  __int64 v6; // x3
  __int64 v7; // x4
  System_Collections_Generic_List_Enumerator_T__o v9; // [xsp+0h] [xbp-50h] BYREF
  System_Collections_Generic_List_Enumerator_T__o v10; // [xsp+18h] [xbp-38h] BYREF

  if ( (byte_4A5B573 & 1) == 0 )
  {
    sub_1B885B0(&System_Collections_Generic_List_Enumerator_PaymentHistoryEntity__TypeInfo);
    sub_1B885B0(&Method_System_Collections_Generic_List_PaymentHistoryEntity__GetEnumerator__);
    byte_4A5B573 = 1;
  }
  EntityList = (System_Collections_Generic_List_object__o *)PaymentHistoryMaster__GetEntityList(this, method);
  if ( !EntityList )
    sub_1B8880C(0LL, v4);
  System_Collections_Generic_List_object___GetEnumerator(
    &v10,
    EntityList,
    (const MethodInfo_34FE32C *)Method_System_Collections_Generic_List_PaymentHistoryEntity__GetEnumerator__);
  v9 = v10;
  return (System_Collections_Generic_IEnumerator_PaymentHistoryEntity__o *)j_il2cpp_value_box_0(
                                                                             System_Collections_Generic_List_Enumerator_PaymentHistoryEntity__TypeInfo,
                                                                             &v9,
                                                                             v5,
                                                                             v6,
                                                                             v7);
}


void __fastcall PaymentHistoryMaster___c___cctor(const MethodInfo *method)
{
  Il2CppObject *v1; // x19
  int32_t v2; // w2
  int32_t v3; // w3

  if ( (byte_4A5B574 & 1) == 0 )
  {
    sub_1B885B0(&PaymentHistoryMaster___c_TypeInfo);
    byte_4A5B574 = 1;
  }
  v1 = (Il2CppObject *)sub_1B887FC(PaymentHistoryMaster___c_TypeInfo);
  System_Object___ctor(v1, 0LL);
  PaymentHistoryMaster___c_TypeInfo->static_fields->__9 = (struct PaymentHistoryMaster___c_o *)v1;
  sub_1B88554(
    (ServantStatusBattleListViewItem_o *)PaymentHistoryMaster___c_TypeInfo->static_fields,
    (int32_t)v1,
    v2,
    v3);
}


void __fastcall PaymentHistoryMaster___c___ctor(PaymentHistoryMaster___c_o *this, const MethodInfo *method)
{
  System_Object___ctor((Il2CppObject *)this, 0LL);
}


PaymentHistoryEntity_o *__fastcall PaymentHistoryMaster___c___GetEntityList_b__1_0(
        PaymentHistoryMaster___c_o *this,
        DataEntityBase_o *entity,
        const MethodInfo *method)
{
  __int64 methodPtr_low; // x10

  if ( (byte_4A5B575 & 1) == 0 )
  {
    sub_1B885B0(&PaymentHistoryEntity_TypeInfo);
    byte_4A5B575 = 1;
  }
  if ( !entity )
    return 0LL;
  methodPtr_low = LOBYTE(PaymentHistoryEntity_TypeInfo->vtable._0_Equals.methodPtr);
  if ( LOBYTE(entity->klass->vtable._0_Equals.methodPtr) < (unsigned int)methodPtr_low )
    return 0LL;
  if ( (PaymentHistoryEntity_c *)entity->klass->_2.typeHierarchy[methodPtr_low - 1] == PaymentHistoryEntity_TypeInfo )
    return (PaymentHistoryEntity_o *)entity;
  return 0LL;
}


int32_t __fastcall PaymentHistoryMaster___c___GetEntityList_b__1_1(
        PaymentHistoryMaster___c_o *this,
        PaymentHistoryEntity_o *a,
        PaymentHistoryEntity_o *b,
        const MethodInfo *method)
{
  if ( !a || !b )
    sub_1B8880C(this, a);
  return a->fields.idx - b->fields.idx;
}