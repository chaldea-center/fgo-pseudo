bool BattleUtility__Above(System_Int32_array *condArray, int32_t value, const MethodInfo *method)
{
  if ( (byte_4E7A55E & 1) == 0 )
  {
    sub_1D0F0B4(&Method_System_Linq_Enumerable_First_int___);
    byte_4E7A55E = 1;
  }
  return !BasicHelper__IsNullOrEmpty((System_Collections_ICollection_o *)condArray, 0)
      && System_Linq_Enumerable__First_int_(
           (System_Collections_Generic_IEnumerable_TSource__o *)condArray,
           (const MethodInfo_3275BE0 *)Method_System_Linq_Enumerable_First_int___) <= value;
}


float BattleUtility__Abs(float f, const MethodInfo *method)
{
  if ( (byte_4E7A55C & 1) == 0 )
  {
    sub_1D0F0B4(&System_Math_TypeInfo);
    byte_4E7A55C = 1;
  }
  if ( !System_Math_TypeInfo->_2.cctor_finished )
    j_il2cpp_runtime_class_init_0(System_Math_TypeInfo);
  return fabsf(f);
}


int32_t BattleUtility__Abs_47881724(int32_t value, const MethodInfo *method)
{
  if ( (byte_4E7A55D & 1) == 0 )
  {
    sub_1D0F0B4(&System_Math_TypeInfo);
    byte_4E7A55D = 1;
  }
  if ( !System_Math_TypeInfo->_2.cctor_finished )
    j_il2cpp_runtime_class_init_0(System_Math_TypeInfo);
  if ( value >= 0 )
    return value;
  else
    return -value;
}


bool BattleUtility__Below(System_Int32_array *condArray, int32_t value, const MethodInfo *method)
{
  if ( (byte_4E7A55F & 1) == 0 )
  {
    sub_1D0F0B4(&Method_System_Linq_Enumerable_First_int___);
    byte_4E7A55F = 1;
  }
  return !BasicHelper__IsNullOrEmpty((System_Collections_ICollection_o *)condArray, 0)
      && System_Linq_Enumerable__First_int_(
           (System_Collections_Generic_IEnumerable_TSource__o *)condArray,
           (const MethodInfo_3275BE0 *)Method_System_Linq_Enumerable_First_int___) >= value;
}


bool BattleUtility__Between(System_Int32_array *condArray, int32_t value, const MethodInfo *method)
{
  _BOOL8 IsNullOrEmpty; // x0
  __int64 v6; // x1

  if ( (byte_4E7A560 & 1) == 0 )
  {
    sub_1D0F0B4(&Method_System_Linq_Enumerable_First_int___);
    sub_1D0F0B4(&Method_System_Linq_Enumerable_Last_int___);
    byte_4E7A560 = 1;
  }
  IsNullOrEmpty = BasicHelper__IsNullOrEmpty((System_Collections_ICollection_o *)condArray, 0);
  if ( IsNullOrEmpty )
    return 0;
  if ( !condArray )
    sub_1D0F30C(IsNullOrEmpty, v6);
  return LODWORD(condArray->max_length) == 2
      && System_Linq_Enumerable__First_int_(
           (System_Collections_Generic_IEnumerable_TSource__o *)condArray,
           (const MethodInfo_3275BE0 *)Method_System_Linq_Enumerable_First_int___) <= value
      && System_Linq_Enumerable__Last_int_(
           (System_Collections_Generic_IEnumerable_TSource__o *)condArray,
           (const MethodInfo_3279DC4 *)Method_System_Linq_Enumerable_Last_int___) >= value;
}


System_Func_int__int__bool__o *BattleUtility__CheckValueSatisfiesCondition(
        System_String_o *condString,
        const MethodInfo *method)
{
  System_String_o *v2; // x19
  __int64 v3; // x0
  intptr_t *v4; // x8
  System_Func_int__int__bool__o *v5; // x19
  bool v6; // w20
  __int64 v7; // x0
  intptr_t *v8; // x9
  bool v9; // w21

  v2 = condString;
  if ( (byte_4E7A561 & 1) == 0 )
  {
    sub_1D0F0B4(&Method_BattleUtility_IsEqualOrHigher__);
    sub_1D0F0B4(&Method_BattleUtility_IsEqualOrLower__);
    sub_1D0F0B4(&Method_BattleUtility_IsEqual__);
    sub_1D0F0B4(&Method_BattleUtility_IsHigher__);
    sub_1D0F0B4(&Method_BattleUtility_IsLower__);
    sub_1D0F0B4(&Method_BattleUtility_IsNotEqual__);
    sub_1D0F0B4(&System_Func_int__int__bool__TypeInfo);
    sub_1D0F0B4(&StringLiteral_1529/*"<>"*/);
    sub_1D0F0B4(&StringLiteral_1740/*"="*/);
    sub_1D0F0B4(&StringLiteral_1493/*"<"*/);
    sub_1D0F0B4(&StringLiteral_1746/*">"*/);
    condString = (System_String_o *)sub_1D0F0B4(&StringLiteral_340/*"!="*/);
    byte_4E7A561 = 1;
  }
  if ( !v2 )
    sub_1D0F30C(condString, method);
  if ( System_String__Contains(v2, (System_String_o *)StringLiteral_340/*"!="*/, 0)
    || System_String__Contains(v2, (System_String_o *)StringLiteral_1529/*"<>"*/, 0) )
  {
    v3 = sub_1D0F300(System_Func_int__int__bool__TypeInfo);
    v4 = (intptr_t *)&Method_BattleUtility_IsNotEqual__;
    v5 = (System_Func_int__int__bool__o *)v3;
  }
  else
  {
    v6 = System_String__Contains(v2, (System_String_o *)StringLiteral_1740/*"="*/, 0);
    if ( System_String__Contains(v2, (System_String_o *)StringLiteral_1746/*">"*/, 0) )
    {
      v7 = sub_1D0F300(System_Func_int__int__bool__TypeInfo);
      v4 = (intptr_t *)&Method_BattleUtility_IsHigher__;
      v8 = (intptr_t *)&Method_BattleUtility_IsEqualOrHigher__;
      v5 = (System_Func_int__int__bool__o *)v7;
    }
    else
    {
      v9 = System_String__Contains(v2, (System_String_o *)StringLiteral_1493/*"<"*/, 0);
      v5 = (System_Func_int__int__bool__o *)sub_1D0F300(System_Func_int__int__bool__TypeInfo);
      if ( !v9 )
      {
        v4 = (intptr_t *)&Method_BattleUtility_IsEqual__;
        goto LABEL_15;
      }
      v4 = (intptr_t *)&Method_BattleUtility_IsLower__;
      v8 = (intptr_t *)&Method_BattleUtility_IsEqualOrLower__;
    }
    if ( v6 )
      v4 = v8;
  }
LABEL_15:
  System_Func_int__int__bool____ctor(v5, 0, *v4, 0);
  return v5;
}


bool BattleUtility__Equal(System_Int32_array *condArray, int32_t value, const MethodInfo *method)
{
  _BOOL8 IsNullOrEmpty; // x0
  __int64 v6; // x1

  IsNullOrEmpty = BasicHelper__IsNullOrEmpty((System_Collections_ICollection_o *)condArray, 0);
  if ( IsNullOrEmpty )
    return 0;
  if ( !condArray )
    sub_1D0F30C(IsNullOrEmpty, v6);
  if ( !LODWORD(condArray->max_length) )
    sub_1D0F314(IsNullOrEmpty);
  return condArray->m_Items[0] == value;
}


int32_t BattleUtility__FloorToInt(float f, const MethodInfo *method)
{
  return BattleUtility__FloorToLong(f, method);
}


int32_t BattleUtility__FloorToInt_47881516(double d, const MethodInfo *method)
{
  int32_t v3; // w9

  if ( (byte_4E7A55A & 1) == 0 )
  {
    sub_1D0F0B4(&System_Math_TypeInfo);
    byte_4E7A55A = 1;
  }
  if ( !System_Math_TypeInfo->_2.cctor_finished )
    j_il2cpp_runtime_class_init_0(System_Math_TypeInfo);
  v3 = vcvtmd_s64_f64(d);
  if ( floor(d) == INFINITY )
    return 0x80000000;
  else
    return v3;
}


int64_t BattleUtility__FloorToLong(double f, const MethodInfo *method)
{
  int64_t v3; // x9

  if ( (byte_4E7A55B & 1) == 0 )
  {
    sub_1D0F0B4(&System_Math_TypeInfo);
    byte_4E7A55B = 1;
  }
  if ( !System_Math_TypeInfo->_2.cctor_finished )
    j_il2cpp_runtime_class_init_0(System_Math_TypeInfo);
  v3 = vcvtmd_s64_f64(f);
  if ( floor(f) == INFINITY )
    return 0x8000000000000000LL;
  else
    return v3;
}


Il2CppObject *BattleUtility__GetDataFromMessagePackString_object_(
        System_String_o *dataString,
        const MethodInfo_3240438 *method)
{
  ByteCompressor_o *v4; // x21
  ByteCompressor_o *v5; // x0
  __int64 v6; // x1

  if ( !method->rgctx_data )
  {
    sub_1D0F0B4(&ByteCompressor_TypeInfo);
    if ( !method->rgctx_data )
      sub_1CE5198(method);
  }
  v4 = (ByteCompressor_o *)sub_1D0F300(ByteCompressor_TypeInfo);
  ByteCompressor___ctor_39749904(v4, dataString, 0);
  if ( !v4 || (v5 = ByteCompressor__AsBase64StrToBytes(v4, 0)) == 0 || (v5 = ByteCompressor__AsGzipToBytes(v5, 0)) == 0 )
    sub_1D0F30C(v5, v6);
  return ByteCompressor__UnpackMiniMessagePack_object_(
           v5,
           (const MethodInfo_324193C *)method->rgctx_data->_0_ByteCompressor_UnpackMiniMessagePack_T_);
}


System_String_o *BattleUtility__GetStringFromMessagePackTargetData_object_(
        Il2CppObject *data,
        const MethodInfo_32404C4 *method)
{
  ByteCompressor_o *v3; // x0
  __int64 v4; // x1

  if ( !method->rgctx_data )
    sub_1CE5198(method);
  v3 = ByteCompressor__ImportMiniMessagePack_object_(data, (const MethodInfo_324175C *)method);
  if ( !v3 || (v3 = ByteCompressor__ToGzip(v3, 0)) == 0 || (v3 = ByteCompressor__ToBase64Str(v3, 0)) == 0 )
    sub_1D0F30C(v3, v4);
  return v3->fields._Str_k__BackingField;
}


bool BattleUtility__IsEqual(int32_t totalValue, int32_t checkValue, const MethodInfo *method)
{
  return totalValue == checkValue;
}


bool BattleUtility__IsEqualOrHigher(int32_t totalValue, int32_t checkValue, const MethodInfo *method)
{
  return totalValue >= checkValue;
}


bool BattleUtility__IsEqualOrLower(int32_t totalValue, int32_t checkValue, const MethodInfo *method)
{
  return totalValue <= checkValue;
}


bool BattleUtility__IsHigher(int32_t totalValue, int32_t checkValue, const MethodInfo *method)
{
  return totalValue > checkValue;
}


bool BattleUtility__IsLower(int32_t totalValue, int32_t checkValue, const MethodInfo *method)
{
  return totalValue < checkValue;
}


bool BattleUtility__IsNotEqual(int32_t totalValue, int32_t checkValue, const MethodInfo *method)
{
  return totalValue != checkValue;
}


// local variable allocation has failed, the output may be wrong!
bool BattleUtility__IsSatisfyValueCond(
        int32_t type,
        System_Int32_array *condArray,
        int32_t value,
        const MethodInfo *method)
{
  bool v6; // w0

  switch ( type )
  {
    case 1:
      return BattleUtility__Above(condArray, value, *(const MethodInfo **)&value);
    case 2:
      return BattleUtility__Below(condArray, value, *(const MethodInfo **)&value);
    case 3:
      return BattleUtility__Between(condArray, value, *(const MethodInfo **)&value);
    case 4:
      v6 = BattleUtility__Between(condArray, value, *(const MethodInfo **)&value);
      return !v6;
    case 5:
      return BattleUtility__Equal(condArray, value, *(const MethodInfo **)&value);
    case 6:
      v6 = BattleUtility__Equal(condArray, value, *(const MethodInfo **)&value);
      return !v6;
    default:
      return 0;
  }
}


float BattleUtility__Lerp(float a, float b, float t, const MethodInfo *method)
{
  float v7; // s0

  if ( (byte_4E7A558 & 1) == 0 )
  {
    sub_1D0F0B4(&System_Math_TypeInfo);
    byte_4E7A558 = 1;
  }
  if ( !System_Math_TypeInfo->_2.cctor_finished )
    j_il2cpp_runtime_class_init_0(System_Math_TypeInfo);
  v7 = System_Math__Max_67084784(0.0, t, 0);
  return (float)((float)(b - a) * System_Math__Min_67085112(v7, 1.0, 0)) + a;
}


double BattleUtility__LerpDouble(double a, double b, float t, const MethodInfo *method)
{
  float v7; // s0

  if ( (byte_4E7A559 & 1) == 0 )
  {
    sub_1D0F0B4(&System_Math_TypeInfo);
    byte_4E7A559 = 1;
  }
  if ( !System_Math_TypeInfo->_2.cctor_finished )
    j_il2cpp_runtime_class_init_0(System_Math_TypeInfo);
  v7 = System_Math__Max_67084784(0.0, t, 0);
  return (b - a) * System_Math__Min_67085112(v7, 1.0, 0) + a;
}


int32_t BattleUtility__ParseConditionValue(System_String_o *condString, const MethodInfo *method)
{
  System_String_o *v3; // x0
  int32_t result; // [xsp+Ch] [xbp-24h] BYREF

  if ( (byte_4E7A562 & 1) == 0 )
  {
    sub_1D0F0B4(&System_Text_RegularExpressions_Regex_TypeInfo);
    sub_1D0F0B4(&StringLiteral_16162/*"[^0-9]"*/);
    sub_1D0F0B4(&StringLiteral_1/*""*/);
    byte_4E7A562 = 1;
  }
  result = 0;
  if ( !System_Text_RegularExpressions_Regex_TypeInfo->_2.cctor_finished )
    j_il2cpp_runtime_class_init_0(System_Text_RegularExpressions_Regex_TypeInfo);
  v3 = System_Text_RegularExpressions_Regex__Replace(
         condString,
         (System_String_o *)StringLiteral_16162/*"[^0-9]"*/,
         (System_String_o *)StringLiteral_1/*""*/,
         0);
  System_Int32__TryParse(v3, &result, 0);
  return result;
}